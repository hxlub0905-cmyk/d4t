# FlexADC Studio 主視窗 — authored 2026-07-28 (M3 收尾).
...
三條資料流（別搞混）
--------------------
1. **編輯流**：UI 事件 → :class:`~flexadc.ui.viewmodel.RecipeModel` 的方法 →
   model 通知 listener → 主視窗刷新 Pipeline/Score 顯示 + 排一次**去抖動
   （300ms）**的預覽。UI 元件自己**不改** model，也不直接呼叫引擎。
2. **預覽流**：:class:`~flexadc.ui.workers.PreviewWorker` 算一顆 defect →
   ``ready`` → 填影像流下拉 / ImageView / 特徵表 / 判定 chip。
3. **試跑流**：:class:`~flexadc.ui.workers.TrialWorker` 跑 N 顆 →
   ``done`` → 直方圖 + bin 摘要 + 狀態列統計。

門檻的「秒回」路徑很重要：拖曳中只用 :func:`~flexadc.ui.viewmodel.rebin`
...
import flexadc.core.steps  # noqa: F401 — 觸發卡片註冊（Qt-free、便宜）
from flexadc.core.pipeline import ParamError, Recipe, get_step, list_steps
...
DEFAULT_CACHE_DIR = os.path.join(os.path.expanduser("~"), ".flexadc", "cache")
...
class StudioWindow(QMainWindow):
    """FlexADC Studio 主視窗（M3 最終組裝）。"""

    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("FlexADC Studio")
    ...
    def load_recipe_path(self, path: Any, sync: bool = False) -> bool:
        ...
        self.setWindowTitle("FlexADC Studio — %s" % self.model.recipe_id)
