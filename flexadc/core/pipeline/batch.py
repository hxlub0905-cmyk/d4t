# FlexADC parallel batch engine — authored 2026-07-28 (M2).
...
# flexadc/core/pipeline/batch.py → 上四層 = repo root（spawn 模式 sys.path 保險）
...
def _init_worker(recipe_json: Dict[str, Any], kind: str,
    ...
    if repo_root and repo_root not in sys.path:
        sys.path.insert(0, repo_root)  # spawn 模式下確保 flexadc 找得到
    pin_cv2_deterministic()
    import flexadc.core.steps  # noqa: F401 — 觸發卡片註冊（fork 模式重複 import 無害）
