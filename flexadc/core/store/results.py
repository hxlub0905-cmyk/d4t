...
- 資料庫路徑由呼叫端決定（CLI 層預設 ``~/.flexadc/runs.db``），這裡不寫死。
- results 列 = :func:`flexadc.core.pipeline.result_to_json_dict` 的 dict
...
from flexadc.core.pipeline.expression import parse_expression
