...
#:
#: ⚠ 這一段 2026-09-03 從 `ui/tree_scene.py` 搬過來 —— F29 C0 把 `OPS`
#: 搬進 core 的時候只搬了程式碼，這段理由留在原地，而它上面那個
#: `_LEAF_PALETTE` 是同一輪該刪沒刪的殘骸。於是十四行說明**掛在一個不相干的
#: 常數上**，讀的人會以為它在講顏色。ruff 的 F811 是從另一頭抓到這件事的。
OPS = (
    (">", "greater than"),
    ("<", "less than"),
    (">=", "at least"),
