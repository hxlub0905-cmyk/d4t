# d4t ingest — authored 2026-08-18 (F11 Region-3 第 2 步).
"""把一份 **GLAS 匯出**掛到已經載好的 lot 上（`<id>_label.png` → 一條影像流）。

分工：**layout 的解析與對位在 GLAS，d4t 只吃它匯出的東西**
（`docs/GLAS-INTERFACE.md`，使用者 2026-08-17 定調）。d4t 裡不會有
...
def read_manifest(export_dir: Any) -> Dict[str, Any]:
    ...
    if not schema.startswith(_SCHEMA_PREFIX):
        raise GlasExportError(
            "%s says schema %r; d4t reads GLAS overlay manifests (%s*). "
...
def region_name_for(layer: str, taken: Any = ()) -> str:
    """GLAS 的 layer 名 → **d4t 的區域名**（`L17/D0` → `L17_D0`）。

    為什麼要有這一支（2026-08-18 對真實匯出實測到的）
    ------------------------------------------------
    真實匯出的三個 layer 名全都含有 d4t 區域名規則不收的字元 —— 那個名字會
