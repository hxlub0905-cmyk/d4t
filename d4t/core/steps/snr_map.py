...
* **這一張**：逐像素的局部 z 值（``(x − 局部平均) / 局部標準差``）—— 輸出是
  一張**圖**，答的是「哪裡突出」。
* **Gray level 的 `snr`**：兩塊區域之間的 ``|Δ| / σ_boxes`` —— 輸出是一個
  **數字**，答的是「這一塊跟那一塊差幾個 σ」（2026-08-21 起 σ 取自參照那一塊
  的**框與框之間**，而且不帶正負號 —— 亮暗的方向由 `delta` 講）。
  ⚠ 以前還有第三個：`roi_snr` 卡（ROI 對周邊 margin 背景）。它在同一天**整張
  刪掉**了，使用者：「我需要的是 GL 比對的 SNR」。
