...
- **全部功能都要能用合成資料開發與驗證。** `tools/make_sample.py` /
  ...
  另外兩支產的是**擬真**的（用來回答「量得準不準」，不只是「跑不跑得動」）：
  `make_mgepi_real.py`（MG×EPI×spacer，缺陷是異材，逼你用 ROI）與
  `make_mgext.py`（MG 從側壁竄出，**凸出量是設計值**，所以量測結果回歸得了）。
  ⚠ **這幾支產出來的資料一律不進版控**（`0822test/` 在 `.gitignore` 裡）——
  固定種子、逐位元組可重現，進版控的是「怎麼產」。
...
- **真實資料的假設必須「可探測」而不是「假設對了就好」。** 這就是 `fab_probe/`
  ...
  預設遮蔽識別碼，所以結果可以貼回開發端。三個待驗證假設在
  [`docs/FAB-VALIDATION.md`](docs/FAB-VALIDATION.md)。
...
>
> #### ⚠ 但水位變了，而那句話是在 888 KB 上確認的（2026-08-24 實測）
>
> | 日期 | bundle | 出處 |
> |---|---|---|
> | 2026-08-16 | 888 KB | 上面那次確認的水位 |
> | 2026-08-19 | 1,386 KB | git |
> | 2026-08-22 | 1,591 KB | git |
...
> 的 88 MB**，也就是 190 MB 裡的 46%。
>
> ⚠ **那個數字十天後就不夠看了。** 2026-09-03 直接量：clone 一份下來
> `size-pack` 是 **385.58 MiB**，而 `git filter-repo --path bundle/
> --invert-paths` 之後剩 **7.23 MiB** —— 也就是 **378 MB（98%）是這一個檔案
> 的歷史副本**，不是 46%。這條曲線是指數的（每一版都是完整一份），所以
> 「先記錄、之後再說」每拖一天都更貴。
>
> #### ✅ 2026-09-03 收掉了，而答案不是上面那兩條
>
> 當時列的兩條路（① 移到 Releases ② 少產幾次）**都不必要**，而其中一條還
> 走不通：**Releases 的附件是 `Content-Disposition: attachment`**，瀏覽器會
> 直接下載而不是顯示文字，而且轉去另一個網域
...
**串成一個檔案仍然是預設做法**（`d4t_bundle.py`：純文字，一個檔案一段）：
一次複製就搬完，比「一個一個檔案點過去」快得多。2026-09-03 之前它是
lzma + base64，代價是內容變成不可讀的 base64；**現在連那個代價都沒有了** ——
包本身就是讀得懂的純文字，而解包程式一直都是可讀的 Python，`--list`
也可以在寫任何檔案之前先列出它要寫什麼。
⚠ 解包程式**兩種格式都認得**（`#ENC text` 與 `#ENC lzma+base64`），
所以已經搬進公司機的舊包照樣解得開。
...
最後一條常被忽略：要驗證 [`docs/FAB-VALIDATION.md`](docs/FAB-VALIDATION.md)
那三個假設，**不需要搬整個專案**。
...
| `show_template.py` | **兩台都可以** | ❌ | 把 recipe 裡的模板（`gc2:` 那一串）存成 PNG —— **一張影像塞在 JSON 的一格裡，打不出來也讀不出來**。Studio 裡看得到（「Edit template & regions…」），這一支是給**沒有 GUI 的時候**用的：檢查一份 recipe、把模板寄給別人看、兩份對照。stdlib-only（自己寫 PNG），所以公司機也跑得動。⚠ 存出來的 PNG 跟原始影像一樣敏感，**不要 commit**（鐵則 8）|
...
見 [`CLAUDE.md`](CLAUDE.md) §4。**測試只在家用機上跑**（公司機沒有 pytest，
也不該把時間花在那裡）：
