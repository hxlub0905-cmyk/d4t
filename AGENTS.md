...
`ADEPT_bundle.py`（lzma + base64 壓成一個 `.py`）解前者，`FILELIST.txt` +
`check_files.py` 解後者。壓縮的代價是內容變成不可讀的 base64；但**解包程式
本身仍然是可讀的 Python**，而且 `--list` 可以在寫任何檔案之前先列出它要寫什麼。

> **不再是限制：GitHub 的 1 MB 顯示上限。**（2026-08-15 使用者確認）
>
> 這裡以前寫著「GitHub 不顯示超過 1 MB 的檔案，所以包一定要壓在 1 MB 以內」，
> 整套分批機制（`make_text_bundle._fit` / `_merge_tail`）都是為它做的。實際上
> **整份文字複製得走**，不依賴 GitHub 把檔案渲染成網頁。
>
> 為什麼要把這段留著而不是刪掉：那個舊前提在程式碼裡留下了真的東西
> （`--split`、`LIMIT_KB`、六批的純文字模式），看到它們的人需要知道
> 「這是為了一個已經退役的限制做的，不是現在的規矩」。**包現在永遠是一個
> 檔案，不管長到多大** —— 不要因為它超過某個數字就把自動分批加回來，那只會
> 讓使用者某天發現要複製兩次，而且檔名跟文件對不上。
...
2. `bundle/ADEPT_bundle.py` —— 整包壓成**一個** `.py`，按複製鈕就搬得走
   （目前約 960 KB；它會跟著 repo 長大，那是預期的，不必也不要為此分批）
...
| `run_tests.py` | **家用機** | ❌ | 跑測試：每個檔案各自一個行程（全套 ~2.5 分鐘，`pytest -q` 是 11 分鐘）|
