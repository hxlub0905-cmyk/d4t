...
> ⚠ **那個包正在逼近上限，而破線沒有任何症狀。** 2026-08-16 量到 962 KB（94%），
> 把 `SESSION_LOG` 的 7 月份與做完的 F7 計畫書封存進 `docs/history/`（那個目錄
> **不進包**）之後回到 888 KB。`release.py` 現在每次產完都會報水位，超過 85%
> 就出聲、超過 1 MB 直接回非零 —— 因為家用機上看不出任何異狀，症狀只會出現在
> 公司機的瀏覽器裡。餘裕不夠時的順序：**先封存**（`docs/history/README.md`），
> 真的塞不下才 `--split` 分批（那會把「1 次複製」變成好幾次）。
> 注意 `make_text_bundle.py` 自己的門檻是 `LIMIT_KB = 900`，比 1 MB 更保守。
