...
> 內容整份改變，git 沒有辦法 delta 壓縮。**328 個版本佔掉 pack 的 88 MB**，
> 也就是 190 MB 裡的 46%。
>
> **這一輪只記錄，不動作**（使用者 2026-08-24：「先不動，我再想」）。要動的話
> 有兩條路，兩條都會改到公司機那條搬運流程，所以要先定調：
> ① 讓 bundle 離開版控（放 Releases），前提是確認公司機看得到那一頁；
> ② 留在版控但**不是每次 commit 都重產**，改成要搬運之前才產一次。
>
