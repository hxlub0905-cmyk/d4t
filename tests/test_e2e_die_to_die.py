# FlexADC M1 端到端驗收 — authored 2026-07-28.
...
另附 CLI（python -m flexadc run）煙霧測試。
"""
...
from flexadc.core.pipeline import Recipe, run_dataset, validate  # noqa: E402
...
def test_cli_run_smoke(synlot, tmp_path):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "flexadc", "run", str(RECIPE), synlot["klarf"],
...
def test_cli_steps_smoke():
    proc = subprocess.run(
        [sys.executable, "-m", "flexadc", "steps"],
