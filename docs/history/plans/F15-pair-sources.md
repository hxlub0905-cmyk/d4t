...
2026-08-20 使用者叫停（「太快了」）：引擎做完且有測試，產品化沒做（缺「對得
對不對」的證據與那份點對點 report）。那一段停在 §16。
**2026-08-25 由 [`F33-ebi-characterization.md`](F33-ebi-characterization.md)
續完** —— 它把叫停的理由一條一條滿足掉之後才續做（配不到的那一顆留下來、
die 內排名、`output_char` 那份點對點 report）。這一份留著是為了**當時為什麼
那樣設計**；現在的行為以 F33 與
[`docs/USING-CHARACTERIZATION.md`](../../USING-CHARACTERIZATION.md) 為準。

歷程：**A / B / C 全部做完**（2026-08-19）＋ **第二輪 F15-2**（用起來的那
...
一句分數表達式（例：`pair_found == 1 and pair_SCORE < 20`）。

### 4.4 配不到的那一顆

> ⚠ **這一節已被 F33 取代**（2026-08-25）。當時的決定連同它為什麼是錯的，
> 一起留在下面 —— 刪掉的話下一個人會再推導一次同樣的結論。

**當時（F15）**：不吐流、`paired = 0`。下游要那條流的卡會失敗 →
**這一顆 `ok=False`，整批照跑**（鐵則 7）。理由是「跟 `load_sidecar` 遇到沒有
label 的那一顆是同一個行為，不發明第二套」。

**為什麼錯**：那個類比只在「缺的東西是輸入」時成立。這裡缺的東西**就是答案**
—— §1.1 那張表的第三列（「EBI 根本沒偵測到」）正是 characterization 要數的
那一類，而它被當成錯誤處理，等於這個功能問不出自己的結論。CSV 上還看不出來：
少了幾列，跟「本來就沒那幾顆」長得一模一樣。

**現在（F33）**：`pair_found = 0`（一律寫）、不吐流、**這一顆繼續走到判定樹**。
下游的 compare 卡靠 `meta["pair_match"]`（`index == -1` ＋ `out` 的流名）安靜
讓路 —— 兩個輸入埠都問，因為配對卡的圖接在哪一個埠是使用者拉的線決定的。
配不到時 `match_dist_nm` **不寫**（以前寫 NaN，而 `NaN != 0` 在判定樹上答
「是」）。`paired` 這個特徵名也一併改成 `pair_found`（它撞了預設流名）。
...
## 16. 為什麼停在這裡（2026-08-20）—— **已於 2026-08-25 由 F33 解除**

> 這一節記的是當時停下來的理由，而那個理由**被滿足了才續做**：缺的那份「點對點
> 包含圖的 report」就是 F33 的 C3（`output_char`），而它需要的兩個資料缺口
> （配不到的要留下來、die 內排名）是 C2 與 C1。見
> [`F33-ebi-characterization.md`](F33-ebi-characterization.md)。
...
這張卡吐 `paired=1`、`ncc_score=0.99`、`match_dist_nm=170`、`peak_ratio=0.05`
—— 那些數字**全都在說「有對到」**，沒有一個在說「**對得對不對**」。而後者是這件
事唯一重要的問題：EBI ↔ RSEM characterization 的結論會拿去下判斷。

一個只會說「我配好了」的配對工具，跟沒有是一樣的 —— 使用者仍然要自己一顆一顆
開圖確認，而那正是他想省掉的事。

**缺的東西有名字：那份 report。** 一顆一列、左右兩張圖、加上那幾個數字，讓人
一眼掃過去就看得出哪幾顆可疑。`pair_probe` 的分布是給**調參數**的人看的，
不是給**檢查這批對不對**的人看的 —— 那是兩件事，而這一輪只做了前面那一件。

### 而且順序錯了

`docs/ROADMAP.md` 的 Compare 那一列本來就寫著「配對機制也在這一段（使用者：
配對是之後的事）」。F15 從那一列裡挑了**最下面那一項**先做，而它上面的東西都
還沒做好。使用者：「我目前 Compare 下面段都還沒做好」。

### 回來的時候接哪裡

1. **證據面**：`paired` 與 `aligned` 並排、疊上配對距離與 peak_ratio，
   逐顆掃得動（不是一次看一顆）。
2. **report**：點對點、含圖，一顆一列。
3. 這兩件都要等 **Compare 段其他卡的形狀定下來**，否則會多發明一套只有這裡用
   的東西 —— 那正是這個 repo 一直在避免的「同一件事兩個地方」。
