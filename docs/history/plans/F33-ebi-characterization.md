...
#### 第一版太淡 —— 而根因是一條**別人的**規矩（使用者：「預覽框太不明顯了」）

`ImageView._paint_marks` 預設「線畫到幾乎看不見（alpha 70）、點畫滿」。那條
規矩是為 **CD 的幾十條掃描線**寫的：線只是說「那個判斷是在哪一列上做的」，
點才是答案，所以線要退到背景去。GLV 也**刻意**靠它（描邊會跟區域框完全重疊，
等於沒畫 —— 2026-08-22 截圖才看到的，F32 又踩了一次）。

H2H 的形狀相反：**少少幾條，而線本身就是答案**。淡化在這裡不是在減少雜訊，
是在藏起唯一的資訊。

**所以不是把畫法改掉**（那會把 GLV 的上緣線變回一條看不見的描邊），而是讓
卡片自己宣告：`Step.marks_solid`（預設 `False`，既有的卡一張都不用動），
`AlignToStep` 設 `True` → 不透明、線 2.2 px、加上四個角點與十字中心點。
「幾條線算少」是卡片自己才知道的事，UI 不猜。

#### 兩個記號要分色（使用者：「預覽也分」）

畫滿之後還有一件事：框與十字**同一個顏色**，畫面上分不出哪個是哪個 ——
而報表那邊早就是紅框＋綠十字了。同一顆 defect 在兩個地方用兩套語言，
使用者要在腦子裡轉換一次。

`labels` 這條路本來就在（一條標記屬於哪一個具名區域 → 顏色跟那個區域的框
一樣），所以**不必發明第五個回傳值**：``!`` 開頭的 label 是**角色**而不是
區域名（沿用 `decide_tree` 的 ``!failed`` / ``!unbinned`` 慣例，而區域名是
識別字、不可能撞到）。

**卡片說角色，UI 挑顏色** —— core 不得 import Qt，而「紅色是什麼紅」是主題的事
（light / dark 兩套值）：

| 角色 | 意思 | 權杖 |
|---|---|---|
| `!match` | 小圖真的對到的那一塊 | `danger`（紅）|
| `!aim` | 機台瞄準的那一點 | `success`（綠）|

跟報表的 `BOX_COLOR` / `AIM_COLOR` 是同一組語言，所以**紅的永遠是「對到哪」、
綠的永遠是「瞄準哪」**，不管你看的是畫面還是那份 HTML。
