...
> **後記（2026-08-23）**：那條「只能在家用機凍」的規矩量過之後是**錯的** ——
> 在容器裡（numpy 2.4.6 / opencv 5.0，比 pin 的新兩個 major）重凍，每一個共有
> 特徵都跟家用機凍的值逐位元組相同，bin 與 score 零差異。**黃金值當天就重凍了**，
> 這條防線恢復。留下來的那一半是「看差異那一步不能跳過」。
> 見 `SESSION_LOG.md`「黃金值重凍」與 `docs/ROADMAP.md`。
