...
**狀態：收斂（2026-08-27）。B1–B5 五批做完。**

⚠ **五批之中四批的數字是錯的，全部朝同一個方向 —— 高估該刪的量。**
清單估「刪 ~282 條」，實際刪掉 **25 條**。事後檢討見 **§12**，先讀那一節再讀下面的清單。
...
| **B4** | ✅ **2026-08-27 做完。** C 組那三支變成常駐檔 ＋ 三條 algo 測試回核心批。**對應刪除沒有做** —— 量過之後那是錯的（見 §10）| 3 搬、11 切出去、0 刪 | 中高（碰到四個零覆蓋模組）|
| **B5** | ✅ **2026-08-27 做完。** D 組其餘刪除 ＋ 文件收尾。**刪了 2 條，不是 ~80** —— 而 B5 真正的產物是量出 UI 批慢在哪（見 §11）| 2 | 中 |

~~**B4 之前不要動 `f7_12`／`f8_cross`**~~：B4 量完之後，這句話的結論從「先做別的」
變成「**不要刪**」—— 見 §10。
...
## 10. B4 的結果：那三支不能刪，只能改名

§3 C 對三支的計畫是「救出不變量，其餘刪掉」（`f7_12` 56→19、`f8_cross` 35→12、
`f7_11_roi` 16→9，合計 ~70 條刪除）。**動手前先量了一次「誰真的踏過那幾個
模組」，結論是那個計畫是錯的。**

用 `coverage --cov-context=test`（方法見 §8）逐檔收，再問每一行「只有誰踏過」：

| 模組 | 被踏過的行 | 只有那一支踏得到 | 其餘 30 幾支踏到的是什麼 |
|---|---:|---:|---|
| `d4t/ui/template_dialog.py`（1,047 行）| 670 | **575**（`f7_12`）| 95 行 —— import 與類別定義 |
| `d4t/ui/cell_canvas.py`（1,086 行）| 598 | **485**（`f7_12`）| 112 行 —— 同上 |
| `d4t/ui/region_check.py`（341 行）| 199 | **45**（`f7_11_roi`）＋ 11（`f8_calibrate`）| 43 行 —— 同上 |

**那不是「有幾條測試碰到這個模組」，是「這個模組唯一的執行覆蓋」。** 照 §3 C
的數字砍掉三分之二，等於把一個 1,047 行的模組交還給運氣 —— 而 §2.3 (b) 這一份
自己就寫著它們是零覆蓋模組。清單同時寫下了硬約束與違反它的計畫。

所以 B4 做的是：

1. **改名成常駐檔**（跟 A 組同一個動作）：
   `f7_12_template` → `test_ui_template_dialog.py`、
   `f7_11_roi` → `test_ui_region_check.py`、
   `f8_cross` → `test_ui_crossing_inspector.py`。
2. **切出一支新的常駐檔**：`test_ui_region_overlay_truth.py`（11 條）。它們原本
   混在交會定位那一輪的驗收裡，但問的是**每一張 Region 卡**都要成立的事 ——
   有幾個框畫幾個、不按鈕就看得到而且參數一改當場變、「這一顆」在哪個框裡、
   只畫選著那張卡的、一張卡好幾個區域要分色而且**名字對不上就不上色**。
3. **三條 algo 測試回核心批**：k× cell 的自週期（`f7_12` 裡唯一一行 Qt 都沒有
   的三條）→ `tests/test_roi_template.py`。
4. **一條都沒刪。**

> **「這一檔有 56 條，太多了」跟「這 56 條裡有幾條是多餘的」是兩個問題。**
> 第一個問題用行數就答得出來，第二個要量。這一份第一版答的是第一個。

⚠ 留下來的真正問題不是條數，是**那兩個模組只有一支測試檔**。要讓它變健康，
該做的是**加**（把 `cell_canvas` 的幾何性質寫成機器撞得到的不變量），不是減。
那件事不在 F39 的範圍裡。

---

## 11. B5 的結果：慢的不是測試條數，是一個視窗洩漏

B5 原本是「D 組其餘刪除（~80 條）」。動手前先問了一個 §6 從來沒問過的問題：
**刪掉這些換到多少秒？**
...
| UI 批逐檔跑的總牆鐘 | 359 秒 |
| D 組十支加起來 | **49 秒（14%）** |
| 其中兩支（`f7_15_reading_load` 20.9s、`f7_24_layout` 12.3s）| 33 秒（**68%**）|
| 其中**三條測試** | 24.7 秒（**50%**）|
...
`win.close()` **不會銷毀它**。試過 `deleteLater()` ＋ `setParent(None)` ＋ `del`
＋ `gc.collect()` ＋ 清掉 `model` 的 listener —— **widget 數一個都沒少**，還有
別的東西握著它（`gc.get_referrers` 追到的最後一個是
`StudioWindow._on_model_changed` 這個 bound method，但清掉它也不夠）。

**這就是 `AGENTS.md` §5 那條「UI 測試不要一個行程跑整套」（整套 1:39:09 vs
逐檔 7 分鐘）的機制**：不只是記憶體，是 O(n²) 的重設樣式。寫進
`docs/PITFALLS.md` 了。

> **在測 A 的地方看到慢，先單獨跑一次 A。** 這一輪如果照原計畫刪掉 80 條，
> 會拿掉真的在守東西的測試、換來一個**看起來變快但成因還在**的 CI ——
> 而下一批新測試會把它一模一樣地長回來。

### 那 80 條為什麼不刪

讀完之後，D 組不是 §3 D 描述的「釘住一輪設計的像素／文字／widget 存在性」。
隨手三條：

* `f13_chrome::every_stage_count_is_readable_in_every_theme` —— 六個階段 × 兩種
  主題逐一算對比，**淺色主題那六個原樣放在自己的淡底上只有 2.8–3.5**。
* `f7_24::fit_shrinks_but_never_blows_up` —— `fit` 是「全部看得完」不是「放到
  最大」，`fitInView` 會把兩張卡的 pipeline 放大成三倍。
* `f7_24::the_replaced_pipeline_panel_is_gone` —— 死碼必須維持死的，理由寫得很
  清楚：「留著的代價不是那 240 行，是每一輪主題工作都要繞過它」。

真的刪的只有兩條（`f13_layout` 的版面那一對），因為它們**整組斷言**在
`f8_ui_polish` 的一條裡，而且驗過「把 bug 放回去兩邊都紅」—— 那是 §8 說的
第一種理由（重複），不是第二種（快照）。
...
五批做完，**四批的數字是錯的**，而且全部朝同一個方向 —— 高估該刪的量：

| 批 | 清單說 | 實得 | 為什麼差 |
|---|---|---|---|
| B1 | 六支改名 | 六支改名 ✅ | — |
| B2 | 兩支整支刪、救 1 條 | 刪了，但要救 **2** 條 | 憑 grep 判斷「有別人蓋到」，實測沒有 |
| B2b | ~25 條純重複 | **8** 條 | 把「重複」與「快照」寫成了同一個數字 |
| B3 | ~30 條搬家 | **18** 條 | 同上（有些是刪不是搬）|
| B4 | ~70 條刪除 | **0** 條 | 那三支是兩個 1,000 行模組**唯一的執行覆蓋** |
| B5 | ~80 條刪除 | **2** 條 | 慢的不是條數，是一個視窗洩漏 |

共同的成因只有一個：**這份清單是讀出來的，不是量出來的。**「這一檔 56 條、
13,128 行、佔 UI 牆鐘 66%」全部是**檔案層級**的數字，而「這一條該不該刪」是
**測試層級**的問題。中間那一步（誰真的在守什麼）從頭到尾沒有做。

三個做得到、而且做了才有答案的量法，都在這一輪長出來：

1. **逐條測試的行覆蓋**（§8）—— 「刪掉它，哪幾行就沒人踏了」。
2. **把 bug 放回去**（§8、§9、§10）—— 「它真的在守嗎」。行覆蓋答不了這個。
3. **單獨跑一次**（§11）—— 「慢的真的是它嗎」。

> **`605 條 → 留 ~323、刪 ~282` 這種一眼看上去很有份量的判定，本身就是警訊。**
> 那個精度不可能來自一次閱讀。實際刪掉的是 **25 條**。

**F39 到此收斂。** 剩下的兩件事都不是「刪測試」：`f21_expr_picker` 隨 Phase 3
一起處理（§5），而 `template_dialog` / `cell_canvas` 只有一支測試檔那件事，
該做的是**加**不是減（§10）。
