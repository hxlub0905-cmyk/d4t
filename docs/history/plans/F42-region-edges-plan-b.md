...
| **B4** | 拆舊路（刪 `region_lines()`）＋ 文件收尾 | ✅ 2026-08-27 |
...
## B4 — 拆舊路 ＋ 收尾（2026-08-27）

### `region_lines()` 刪掉，畫布直接讀 `edge_lines()`
...
留著的理由寫在它的 docstring 上，而且有一條便利貼測試守著
（`test_the_ui_and_the_core_agree_on_who_defines_a_region`）：核心的
`recipe._region_producer`（遷移補線時找來源的那一支）用的是**同一個語意**，
兩邊要動就一起動。

> 這一條是照 `CLAUDE.md` §5 的前例做的：`algo/period.py` 只剩一個呼叫者的那
> 一小時，差一步就被當成死碼清掉。沒有呼叫者的東西要嘛刪掉，要嘛留一張便利貼
> ——「留著但沒有人守」是最短命的那一種。

### `doctor.py`：recipe 格式

新增一項（**非必要**，△ 不是 ✗）：掃 `recipes\`（或命令列給的路徑），
`version < 2` 的檔案提示「用 Studio 開起來按一次 Ctrl+S」，並告訴手寫 recipe
的人區域現在要寫成一條線。

判準是**版本號**，跟遷移本身一樣 —— 而那也是唯一做得到的：doctor 是
stdlib-only（`AGENTS.md` §3，它要在「套件還沒裝好」的機器上跑），
問不到「哪一格是區域」。`RECIPE_VERSION` 因此在 doctor 裡抄了一份字面值，
而 `test_the_doctor_and_the_engine_agree_on_the_recipe_version` 付那份代價。

### 文件

**改掉舊句子，不疊刪除線**（常駐文件只寫現況，為什麼變留在 history —— 使用者
定調）：

| 檔案 | 改了什麼 |
|---|---|
| `CLAUDE.md` 鐵則 10 | 「區域線不存進 `recipe.edges`」→ 現況（線就是儲存、順序只看線、`duplicate-region`、手寫要寫線）|
| `docs/ARCHITECTURE.md` §資料模型 | 同上，並指向這份計畫書 |
| `docs/ROADMAP.md` 三種通道那張表 | 「虛線（推導）」→「虛線（存在 `edges` 裡）」|
| `docs/PITFALLS.md` | **加一條**：Region 卡排在量測卡右邊的那個 bug，以及它的通則 |
| `docs/history/plans/F12-region-edges.md` | **一個字都沒改**（它是歷史）|

`docs/USING-*.md` 沒有手寫 recipe 的 JSON 範例（查過：整個 `docs/` 沒有
```json 區塊），所以那一項沒有東西要改。
...
（`execution_order` 收成只看線）就失效了 —— 中間隔了八天，而沒有人回頭重讀
那個決定。第七個「跑得完、有數字、而且是錯的」就是那八天的利息。

### 最終驗收

* 核心 **2430** 綠、**62 支 UI 測試逐檔綠**、黃金值三份**逐項相同**。
* `tools/freeze_golden.py --check` 從 B0 到 B4 每一段都跑過，**沒有一次有差異**
  —— 整個方案 B 是純重構這件事，是逐段證明的，不是最後才驗的。
* CI 時間沒有變差（核心 108–112 秒，與這一輪開始前的 115 秒同一個區間）。
