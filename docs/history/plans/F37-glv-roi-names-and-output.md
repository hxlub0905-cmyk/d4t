...
| # | 題目 | 狀態 |
|---|---|---|
| A1 | 名字加家族 tag ＋ 遷移補到第四條路 | ✅ |
| A2 | 前綴仍是條件式，但改名不再是安靜的（(ii)） | ✅ |
| A3 | `the other regions` 長出一條真的線 | ✅ |
| A4 | 特徵名上下標＋顏色 | ✅ |
| B1 | 合併 A（`output_image` → `output_bundle`）| ✅ |
| B1 | 合併 B（`output_char` → `output_bundle`）| ❌ **不合**（使用者定調，理由見 §5.1）|
| B2 | 產物內容收斂（§6 那五項）| ✅ 四項做了，一項查證後**不動**（見 §6 的後記）|

### 5.1 為什麼最後沒有合 `output_char`（2026-08-26 定調）

使用者：「先不合」。而**帳在做完 B2 之後才算得清楚**，值得記下來：

當初提議合併的理由是「三張寫資料夾的卡共用九成參數與同樣四個檔名」。
B2 把那個「九成」**真的收掉了** —— 報表開頭（`export_html._page_head`）、
CSS 與跳脫、ROI 框那三格（`roi_draw_specs`）、`rank_by`（`rank_by_spec`）、
`limit` 的 `0 ＝ 全部`（`LIMIT_ZERO_HELP`）、路徑檢查（`_OutputStep.path_issue`）
現在全部只有一份。

於是合併**能再省下的只剩「卡片庫少一列」**，而代價沒有變小：那張卡的 help
與測試要同時描述兩種版面。而版面的差別是真的：

| | 報表資料夾 | characterization |
|---|---|---|
| 為幾顆設計 | 6000 | 30 |
| 表格裡的圖 | **不放**（6000 個 `<img>` 會讓 DOM 很鈍）| **每一列兩張** |
| 怎麼看圖 | 點一列換右邊那張（整份只有一個 `<img>`）| 圖就在列上，一眼掃完 |

> **這一項的教訓不是「合併是錯的」，是「重複要先量、而量的時機會改變答案」。**
> 同一個提議在 B2 之前與之後成本效益不同 —— 因為中間那一步把「重複」本身
> 拿掉了。先收共用的部分，再決定要不要合併卡片，順序反過來會多做工。

⚠ **這個決定有一支測試守著**（`tests/test_output_convergence.py::
test_the_two_layouts_are_still_two_functions`）。哪天真的要合，先回來刪它
並在新的計畫書上寫下為什麼 —— 不要讓它安靜地消失。

下面是當初的分析與提案，**原樣留著** —— 它記著每一項的代價與反面論證，
而那正是「當初為什麼這樣下」要查的東西。
...
### §6 的後記（2026-08-26，做完之後）

| # | 結果 |
|---|---|
| 1 CSV 的規則 | **不動**（查證的結論，不是漏掉）。三張卡走同一支 `write_csv`、欄位逐字相同；`include_features` 刻意只留在 `output_csv` 上 —— 那是交付物，資料夾裡那份是報表的隨附檔，關掉之後幾乎是空的。把那一格補到另外兩張卡上是**加旋鈕**，不是收斂。理由寫進 `steps/output.py` 的模組說明，免得下一個人來「統一」它 |
| 2 HTML 版面 | **收的不是版面，是一樣的那一段**。原本寫「收成一支 + 版面參數」是錯的 —— 兩份版面的取捨在 6000 顆與 30 顆是**反過來的**（那正是 B1(b) 的同一個論證）。真正的重複是**開頭 14 行逐字相同**（標題、幾顆、bins 摘要、判定那一段），抽成 `_page_head`。並且加了一支**守著「不要合併」**的測試 |
| 3 `limit` 的意思 | ✅ 兩張卡都 `min=0`、`0 ＝ 全部`，共用同一句 help（`LIMIT_ZERO_HELP`）。⚠ 預設值刻意仍然不同（0 vs 200）—— **約定共用，取捨各自保留**。順手修掉一刀：`ordered[:0]` 是「一張圖都沒有」，正好是「不限」的相反 |
| 4 char 的 ROI 框 | ✅ 補上，而且**只畫在真的被量的那條流上**（換一條流當背景時框的座標就沒有意義，同 `_defect_marks` 的「不猜」） |
| 5 路徑那一格 | ✅ 收成 `_OutputStep.path_issue` 一支。真正的收穫不是少了兩份重複，是**時機**：那句話以前住在 `run_batch` 裡，要跑完一整批才聽得到；現在在畫布上就掛得出警示標記 |
