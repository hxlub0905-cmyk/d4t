...
報一個「單元內歧義」旗標，並檢查**把區域整組平移 1/k 之後是不是落回自己** ——
是就無害，不是就該警告「這一顆可能落在另一半，而你在兩半上標的東西不一樣」。
那是唯一還會安靜出錯的路。
...
改成**間距先取整數，再從第一個錨點一路排下去**（`cellrois._array_boxes_px`）：
`[30, 30, 30, 30, 30, 30, 30, 30]`。代價是最後一個框可能離第二個錨點差幾個像素
（9 個框、真實間距 29.5 差 4 px），而那個差**看得見** —— 預覽就在畫面上，提示列
也把用掉的 pitch 講出來（`9 rectangles · pitch 30 px across — every gap is the
same.`）。pitch 是設計常數，它本來就該是同一個數字。
