...
| 2 | `roi_template` **12 個參數、一個小標題都沒有** | 同一段裡兩張卡兩種待遇。而它的四個 `roi_x/y/w/h` 是**手打的數字**：框的位置只能用數字猜，而 Enhance-UI-A 剛證明了「一邊看影像一邊決定」才是使用者真的在做的事 |
| 3 | 三張卡的**框畫在預覽上，但拖不動** | `region_overlay` 只是畫（F7-11）。ROI 段的本業就是「框在對的地方」，而畫布上唯一能改它的方式是回設定區打四個數字 |
| 4 | `roi_mask` 的 `regions` 是**自由文字** | 要打的字必須跟上游卡片的 `roi_out` 一字不差。打錯的話 lint 會抓（`unknown-region`），但它本來可以是一個下拉 —— 上游有哪些名字，程式知道 |

#### 3.3.2 三題要你定（Region-1 動手之前）
...
**我的建議：B。** 理由是曼哈頓佈局**本來就是矩形的集合**，所以 B 不是近似、
是等價的表示法；而它不動資料模型，就不必去改五個下游。真的遇到斜的或圓的
（GLAS 的樣本到了就知道）再升級成 C —— **C 的入口是「NamedROI 多一個選填欄位」，
B 做完也不會擋住它**。

##### 第二題：`roi_cross` 與 `roi_template` 要不要在這一輪一起整理

它們是**既有功能**，不整理也跑得動。但 §3.3.1 的第 2、3 項（手打的框、拖不動的框）
是同一件事的兩半，而那件事 Enhance-UI-A 已經證明有用（把參數畫在影像上）。
**建議：`roi_from_mask` 先做（它是新能力），框可以拖是第二輪。**
...
#### 3.3.3 Region-1（`roi_from_mask`）的規格 —— 第一題定了才動手
