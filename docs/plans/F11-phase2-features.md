...
| 4 | Studio：~~掛匯出的入口~~（✅ 提前做了）、`layers` 的表格編輯器、儀表（label 上色預覽 + 「這一顆對到幾個區域」）| |
...
`ChannelMapField` 這個**編輯器**是照「一列一張圖、左欄是頁碼」設計的
（`min_rows` 吃的是「一顆有幾張圖」）。label id 不是頁碼，列數要來自**匯出的
`label_map`**。值的格式共用是對的，**widget 要一個變體** —— 現在直接用會讓表格
長出跟 label 無關的列數，那是「畫布說謊」那一類。
