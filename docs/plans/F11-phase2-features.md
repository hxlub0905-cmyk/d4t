...
| 檢查 | 結果 |
|---|---|
| 新測試 | `tests/test_ui_f11_card_aids.py`（12 支：A 七支、H 五支，各含一支從真實預覽／真實 trial 走完整條路的）|
| 核心測試 | 1128 passed |
| 黃金值 | 三組 22 顆逐項相同（`ParamSpec.extent` 只是宣告，不進運算）|
| 剩下的 | C（曲線墊直方圖）、E（磨掉的是雜訊還是訊號）、B（兩條流可比嗎，要重凍黃金值）|
