...
> ### ⚠ 這一份現在有兩種讀法，不要混
>
> * **還活著的那一半**：§0 的「一張卡要回答的四題」與「設定要放哪」是**方法**，
>   加任何一張新卡都適用；以及 Compare 段那一段（唯一還沒收斂的）。
> * **已經是紀錄的那一半**：Input / Enhance / Region / Measure / ADC / Output 六段
>   的逐張討論。它們寫的是**當時**的決定，而其中有些後來被推翻了 —— 例如這一份
>   還在講 `roi_mask` 與 `normalize` 的 `use_within`（兩個 2026-09-02 一起刪掉）、
...
**解法（✅ 1 與 3 做了 2026-08-18；2 還沒）**：

1. **確定度摺在「自週期」上，位置仍然摺在整個 cell 上。** 兩者摺在不同週期是刻意
   的 —— 框是標在整個 cell 上的（位置要知道對到 cell 的哪裡），但那 k 個峰是
   **同一個答案的複本**（確定度不該把它們算成「另一個答案」）。
   模板字串因此帶著自週期：`gc1:<w>x<h>:<b64>` → `gc2:<w>x<h>:<sx>x<sy>:<b64>`，
   舊的照讀（那時候自週期視同 cell 尺寸 ＝ **與以前逐位元組相同**，黃金值不動）。
   實測修好之後：1× 0.367–0.608、2× 0.367–0.607、3× 0.366–0.606。
3. 編輯器的摘要在選了 k× cell 的**當下**就講：「this cell holds 2 copies of a
   40 x 240 px repeat, so a patch cannot tell which copy it is on — fine if you
   mark the same thing on each, not fine if they differ」。

##### 自週期是**驗證**出來的，不是估出來的

拿 `period.estimate_period` 量那個 cell 會得到**假答案**：實測 MG/EPI 的 cell 回
20 px，因為兩條亮邊剛好間隔 20 —— 但圖案在 20 px 上並不重複（一段是 MG、一段是
EPI）。估週期問的是「哪個間距有起伏」，這裡要問的是**捲過去之後整張圖對不對得
起來**，而那是一個可以直接驗的問題。

所以只試 `1/k`（k = 2…8 且整除），取通過的最小單元。實測分得很開：真的自週期
NCC 0.995–0.998，其餘除數 ≤ 0.75（門檻 0.9）。

⚠ 還要一道鑑別：**平的那一軸對任何位移都相似**。一維 layout 的 Y 軸就是平的
（cell 高 = 整張影像），第一版因此回報「自週期 30 px」—— 一個純粹的假答案。
所以捲**半個**候選週期必須明顯不像（真週期 0.998 vs 0.52；平軸兩個都 ≈ 1）。

##### 第 2 項（✅ 2026-08-18）：把「要不要緊」變成一個**可以判定**的問題

k× cell 的歧義**只有在兩份上標的東西不一樣時才要緊**。而那不是一句要使用者
自己小心的叮嚀 —— 它是可以算的：**把每個區域的框整組平移 1/k，看集合是不是
映射回自己**（`cellrois.regions_repeat_at`，環狀平移，而且名字要對上 ——
ROI1 平移之後落到 ROI2 的位置不算，那是兩個不同的量測）。

* 落回自己 → 兩個答案都對，講一句「your regions are the same on every copy,
  so either answer is right」就好。
* 沒落回自己 → **這條路上最後一個會安靜出錯的地方**（確定度已經正確地不再
  因為 k× 扣分，所以不會有別的東西擋它）。編輯器即時警告，卡片的
  `configuration_issues` 在**跑之前**也擋一次 —— 它是設定的性質（模板 × 區域），
  不是每一顆的執行期行為。

#### 3.3.8 編輯器第五輪：等距的 pitch ＋ 放下去之前的預覽（✅ 2026-08-18）

> 「MULTI ADD 像素 PERIOD 會不穩定，舉例有 9 個 BOX，PERIOD 我去看表格有時是 29
> 有時是 30，這樣會造成在畫布顯示也對不齊，這有解嗎？」

有，而且原因很單純：**先算小數的中心、再逐個四捨五入到整數像素**。真實間距 29.5
的時候，「每個都最接近」的必然結果就是 29／30 交錯 —— 每一個框單獨看都對，整排
看起來卻是歪的。實測 9 個框：`[29, 30, 30, 29, 30, 29, 30, 30]`。

第一版的處置是「間距先取整、再從第一個錨點排下去」。**那個修法是錯的**，而使用者
當場問回來：

> 「multi add 的 pitch 改成非整數會比較好嗎（主要是畫面，因為 patch 對 template
> 是對圖）？」

對，理由是**誤差會累積**：整數 30 對上真實的 29.5，第 9 個框偏 4 px —— 在 6 px 寬
的 EPI 上那是量到隔壁去了。精確的 29.5 則是每一個框都在真實位置的半個像素內。

而真正的重點是**該在哪裡取整**：`roi_boxes_in_patch` 本來就會在落到 patch 上時
`round(x·cx − phase)`，而每一顆 patch 的相位不同 —— 同一個小數座標在不同 patch 上
落到不同的整數像素，**那正是對的**，它追的是那一顆上結構真正的位置。
**編輯器先取整等於把誤差烤進去。**
...
| 工具 | 座標 | 為什麼 |
|---|---|---|
| Drag / Click / Paint | 整數 | 使用者對著一個像素邊在瞄，整數就是他的意圖 |
| **Multi add** | **精確小數**（大小仍是整數 px）| pitch 是設計常數，它很少剛好是整數 |
| 表格 | 有小數就顯示小數（`13.5`）| **表格說謊才是「不穩定」的來源** —— 顯示成 29／30 交錯的其實是同一個 29.5 |
| 提示列 | `pitch 29.6 px … every gap is exactly this` | 那才是使用者要的那個數字，而且它是穩定的 |

> 「MULTI-ADD 在還沒點下去前也幫我有個 box 預覽（方便對齊擺放）。」

`hover_box()`：click／array 兩支工具在游標下畫出**按下去會長出來的那個框**，
中心畫一個十字（「按下去的那一點是框的中心」在放下之前就要看得見）。放了第一個
錨點之後改成預覽整片，不再多一個孤兒框；游標離開畫布就消失（畫布不能說謊）。
...
##### 那張比較卡（✅ 2026-08-18，`roi_compare`）—— 見 §3.5.1
...
#### 3.3.10 編輯器第六輪：跨區域選取、對著游標縮放（✅ 2026-08-18）

| # | 回報 | 處置 |
|---|---|---|
| 1 | 「Multi add 只有點左上會有預覽，我希望點右下也會有預覽（中間的 box 也要）」| 放了第一個錨點之後，**游標就是暫時的第二個錨點** —— 整片（含中間那些）在按下去之前就看得到。提示列同步講出數量與 pitch，但「Add」還不能按（那一片還沒定下來）|
| 2 | 「顯示上 W→H→X→下箭頭（這個應該是要 Y）」| `×`／`↓` → `X`／`Y`。`×` 被讀成乘號 |
| 3 | 「Select 框選可以跨 Region（這樣才可以讓不同 region 對齊）」| 選取從「框索引」改成 **`(區域, 框)`** —— 套索、Ctrl＋點、對齊、刪除、方向鍵全部跨區域 |
| 4 | 「滾輪 zoom in 是朝當前滑鼠位置，而不是畫面中間」| 記下游標下那一點的 cell 座標，縮放後用 pan 移回原位。**「我指著的東西不要跑掉」是縮放唯一該保證的事** |
| 5 | 「八角的拉選框預設不要框起來，只有 select 點到或框到時才顯示」| 兩件事：multi add 加完**不再全選**；而且**只有選單一個框才畫把手** —— 一個把手拉不動二十個框，「選中了」用外框加粗就講完了 |
| 6 | 「可以框選 ROI 後按上下左右鍵一起移動」| 本來就會動，但**焦點會跑掉**（按過側欄的鈕之後鍵盤沒反應，而畫面上框看起來還選著）。點畫布就把焦點抓回來 |

第 3 項是資料結構的改動（`_selection: List[int]` → `List[Tuple[int, int]]`），
其餘五項疊在它上面。命中判定**目前這個區域先接** —— 兩個區域的框疊在一起時，
正在編的那一個要優先，否則點下去會跳到另一個區域。

##### `margin`（確定度）預設不再當門檻

> 「目前 certainty 還是會太小，還是拿掉？」

**拿掉的是門檻，不是數字**（`min_margin` 預設 0.05 → 0）：

* **數字要留著** —— 它是唯一講得出「這一顆可能落在另一份上」的東西。
* **門檻預設 0** —— 週期性強的 layout 上這個數字對每一顆都小，拿它擋顆數，
  擋掉的多半是好的。

而「落在另一份上要不要緊」**已經有一個確定的答案了**（§3.3.7 第 2 項：把區域整組
平移 1/k 看落不落回自己）。有了那個檢查，margin 就不必再當一道盲目的閘門 ——
它退回它本來的身分：一個診斷數字。

⚠ 三組黃金值都沒有用到 `roi_template`，所以這個預設值改動沒有動到它們。要擋的
人照樣擋得住，只是要自己把門檻拉起來。

#### 3.3.9 Region-2（`roi_cross` / Profile）—— 2a：用圖取代文字（✅ 2026-08-18）

##### 使用者定調

> 「理想上我不希望 profile 設定頁面那麼多 **文字**，能用圖就用圖（like template
> 簡單的設定方式）。」

所以 Region-2 **不是刪參數，是把文字換成圖** —— 跟 Region-1 同一個手法
（那一輪把「四個 0–1 的數字」換成「在 cell 上畫」）。

##### 24 格其實分三種，而每一種換成圖的方式不一樣

| 種類 | 哪幾格 | 換成什麼 |
|---|---|---|
| **幾何選擇**（不看資料，畫得出來）| `place`(5) `side`(3) `fill_rule`(3) | **圖示列** ← 2a 做的 |
| **看資料的選擇** | `select` `kinds` `pitch` `pitch_2` `width` `sensitivity` ×2 | 畫在影像上（2b）|
| **框的三個尺寸** | `box_size` `gap` `inset` | 在框上拖（2b）|

##### 2a 做了什麼

新的 `icon_choice` 參數型別（`ParamSpec.icons` ＋ `choice_help`），表單看到它就畫
一排 `IconButton` 而不是下拉。**只有「不看資料就畫得出來」的選項適用** ——
`vertical_select`（最亮的那一組是哪一組）看的是這張影像，畫不出通用的圖示，
那種要畫在影像上。

十一個新圖示，每一個都是**一張小小的版圖**：淡的是條紋（背景，「哪裡有材質」），
實心的是框（那才是這個選項在講的東西）。十一顆並排時唯一的差別就是亮塊在哪 ——
那正好就是這些選項唯一的差別。

⚠ **這些圖要在 21 px 下讀得出來，而第一版不行。** 第一版畫得很細（薄框
`w*0.07` ＝ 1.5 px、兩根直條紋加一條橫帶），render 出來五個 `place` 幾乎一模一樣。
`fill_rule` 的第一版把「缺的那一格」畫成淡色實心 —— 在那個尺寸下仍然讀起來像
一根，於是 `fill` 與 `skip` 長得一樣。改成虛線外框才分得開。
**在這個尺寸下「精確」跟「看得懂」是衝突的，而看得懂才是這一輪的目標。**

（這兩個問題都是**把 widget render 成 PNG 看出來的**，不是想出來的。）

##### 一個越界，收回了

我順手把 `kinds` / `width` / `fill_rule` 也收進 advanced（24 → 11 可見）。
`test_ui_f8_advanced.py::test_the_rows_that_matter_are_still_there` 當場擋下來 ——
那條測試鎖著 F8 第六輪的判斷：**它們是「非答不可」的**。

而且使用者要的是「用圖取代文字」，不是「把東西藏起來」。收回，只留
`pitch_2` 收進 advanced（help 本來就寫「leave it 0 otherwise」）。
24 → **16 可見**，其中三行從一句英文變成一排圖。

##### `core` 不 import Qt，所以圖示名對不對只有 UI 那側驗得了

`test_ui_widgets.py::test_every_icon_a_card_declares_really_exists` 掃過每一張卡
宣告的每一個圖示名。對不上的症狀是 `IconButton` 直接
`ValueError: unknown icon` —— 那張卡整個打不開，而且是在使用者點下去的時候才炸。

##### 2b：點曲線上的一根條紋 = 選那一種材質（✅ 2026-08-18）

`vertical_select` / `horizontal_select` 是一個 rank 下拉（`brightest` /
`second_brightest` / `darkest` …）。**那些詞本身不告訴使用者任何事** ——
「哪一組是第二亮的」是一個只有看圖才答得出來的問題，而圖就在旁邊那塊儀表上。

| | |
|---|---|
| 引擎 | `select_bands` 本來就在算分群，算完就丟掉。抽出 `algo/grid.band_groups`（兩邊共用，規則只有一份），並把 `levels` / `groups` / `group_picked` / `group_rules` 放進 `meta` |
| 面板 | 底部一條**分群色帶**：每一群一個顏色，選中的那一群畫實、其餘很淡 |
| 互動 | **點一下 = 選那一種材質**（`select_requested` → Studio 的 `set_param`，走跟「量給我填」同一條路，所以 Ctrl+Z 撤得掉）；**拖一段 = 量尺**（既有的，不變）|

##### 為什麼分群是引擎給的、而不是面板自己算

分幾群**取決於現在挑的是第幾亮**（`k = max(rank + 2, kinds)`）。面板自己分的話
會跟引擎用的那一組不一樣，而症狀是「畫面上那一群」跟「真的被挑到的那一群」
差一格 —— 那正是 `ProfilePanel` 檔頭已經寫過的那條規矩（畫的資料來自引擎那一次
計算）。

##### 兩個 render 出來才發現的

* **整段上色會把既有的塗色淹掉。** 第一版把每一段都依群上色，結果「現在用的是
  哪一組」（既有的 `accent_bg` 塗色）只剩 alpha 的差別，而且畫面很吵。兩條既有
  測試當場紅（它們量「中間那一列被塗了幾欄」，而整段上色把對照組洗掉了）。
  改成**底部一條細色帶**：色帶回答「有幾種、哪一種是哪一種」，塗色回答「現在
  用的是哪一種」—— 兩個問題，兩個位置。
* **空隙那一群通常最寬**，照一樣的濃度畫會把整條色帶佔滿，真正要看的那幾根反而
  變成小點。沒選中的降到 alpha 70、厚度減半才對得起來。

##### 手勢怎麼分

同一個手勢兩種意思會很糟，所以用「有沒有移動」分：**點一下（≤1.5 個取樣點）
＝ 選材質，拖一段 ＝ 量尺**。那是使用者本來就分得出來的兩件事，而尺本來就要拖過
一段才有意義（0 寬的尺量不出東西）。

##### 「在影像上拖框邊」—— **不做，而且理由要記下來**（使用者定調 2026-08-18）

2a 這一輪結尾我留了一條尾巴：「框的三個尺寸（`box_size` / `gap` / `inset`）還是
數字，它們該在影像上的框邊直接拖。」使用者一句話否決：

> 「Region2 拖框邊應該沒意義吧～ 你拖這張 其他張怎麼辦」

**他是對的，而這正好是 Template 與 Profile 的分界，值得寫死在這裡。**

Template 拖得動，是因為畫布上那張 Golden Cell **就是所有 defect 共用的那一個
東西** —— 你在 GC 上畫的框，200 顆全部繼承它（`roi_boxes_in_patch` 把它鋪過去）。
Profile **沒有這種東西**：畫面上那些框是「這一顆的結果」，逐顆不同（那正是這張卡
存在的理由 —— 框跟著結構跑，不是固定在畫面上）。**拖一個結果等於在改輸出。**

唯一自圓其說的讀法是「拖的其實是 `gap` 這個純量」（離條紋邊界幾個 px，每一顆都
一樣）。但那樣一放開手，別的 patch 上的框會落在別的地方，而你在這一顆上看到它剛好
停在那裡只是因為這一顆的條紋在那裡 —— **使用者拖完的期待跟實際發生的事不一致**，
那比留著數字框更糟（推廣鐵則）。

推得出來的通則：**可以用拖的，只有「所有 defect 共用的那一個物件」上的東西。**
Region-3（GDS label map）跟 Template 同一邊（那張 map 是共用的）；Profile 跟未來
任何「逐顆重算」的定位法都在另一邊，它們的設定只能是**跨顆成立的規則**。

那三個數字如果要更好懂，方向是**把它們畫成一張剖面圖**（跟 `place` 那五顆圖示同
一招：一根條紋 + 三個標註的尺寸），不是讓人去拖預覽。使用者沒有要求，不要主動做。

##### 既有的資產（做 2b 之前先查的）

##### 當初的盤點

既有的資產比預期多，**圖已經有了，只是不在設定的地方**：`CrossInspector` 的兩條
profile 曲線、拖曳量測尺（F8 第五輪）、一鍵校正整批的 pitch/width（F8 第七輪）。
那跟 Region-1 是同一個病 —— **參照物不在場**。

使用者補了一句「原來 UI 放 profile 的區域你也可以運用」，所以 2b 不一定要開對話框，
右下角那塊儀表就是它的家。

#### 3.3.11 Region-2c：**交會是特例，不是前提**（✅ 2026-08-18）
...
他是對的，而**過度貼合的位置指得出來，是一行**：`algo/grid.locate_crossings`
...
那一行是從需求（直的 MG × 橫的 EPI）抄下來的**形狀**，不是這套演算法的形狀。
投影量的是「這個方向每一根線在哪」—— 一個方向就答得出來，第二個方向是**加上去
的限制**，不是前提。於是一張密集 line/space 的 patch（只有一個方向有結構，這在
EBI 上很常見）進到這張卡只會拿到一句「no flat stripes to lock onto」然後退回整張
圖、每一顆標 `locate_ok = 0`。**一件做得到的事被卡片的形狀擋住了**，而使用者感覺
到的「用不太到」就是這個。

##### 為什麼不是「收起來」

第一直覺是照 `align` 的辦法把它加進 `scope.HIDDEN_STEPS`。不對 ——
`align` 收起來是因為**它做的事本身是錯的**（量到雜訊＋缺陷偏移、只有 ref 被重採
樣）。這張卡做的事沒有錯，只是**問了一個不必問的問題**。收起來會連那一整套投影
定位（次像素邊界、排名挑材質、已知 pitch 補線、CPODE 的 `skip_clear`、一鍵校正整
批）一起收掉，而那些東西沒有第二個家。

##### 做法：一個方向 = 另一個方向有一根「蓋滿整張圖的條紋」

新參數 `directions`（`both` / `upright` / `flat`，圖示列，section 1 第二格）。
單方向的實作**不是**在 `cross_boxes` 裡加分支，而是給沒在用的那一軸一個退化的
`StripeSet`（`algo/grid.open_axis`）：`selected = [(0, length)]`。

交會的幾何一行都不用改 —— 跟滿版帶子交會出來的正好就是滿版的框。這個選擇是刻意
的：五種放法 × 兩軸各自可能是被貼住的或當界線的，加分支等於把那張表變成兩張，
而只有走單軸時才會用到的那一半沒有人在看。

於是三種用法走**同一段程式碼**：

| `directions` | `place` | 拿到什麼 | 就是當初的 |
|---|---|---|---|
| `upright` | `crossing` | 每一根直線上一條**滿版**的帶子 | `roi_profile`（39b9fea 刪掉的那張）|
| `upright` | `beside_vertical` | 每一根直線左右各一條滿版的窄帶 | —— |
| `both` | `beside_vertical` | MG × EPI 交界、落在 EPI 那一側 | F8 的需求 |

##### 三個「沒在看 ≠ 找不到」

同一個病的三個發作點，全部是**平的曲線在這套 UI 裡一直代表「這裡沒東西」**：

1. **閘門**：沒在用的方向信心是 0，而 0 < `min_confidence`。拿一個沒問的問題去
   否決一顆算得出來的 defect —— `conf` 改成只取**在用的**那幾軸的 min，
   `bands < 2` 與 `trust_note` 同理。
2. **曲線面板**：那條平線畫出來會被讀成「去調敏感度」，意思完全相反，而且佔掉
   在看的那條一半的高度 → `CrossInspector` 把它 `setVisible(False)`。
   面板靠 `ctx.meta["crossings"][name]["directions"]` 判斷（**引擎那一次計算**
   帶出來的，UI 不自己推）。
3. **摘要**：`flat pitch 0.0 px` 是一個看起來像量測失敗的假數字 → 不報。

##### 方向與放法對不起來，要在跑之前講

「只看直的」＋「框放在兩根**橫**條紋之間」是空集合。它不報錯，它安靜地產出零個
框、退回整張圖 —— 而畫面上看起來就跟「這批圖沒有結構」一模一樣，於是使用者會去
調敏感度，因為錯誤訊息指的是那裡。兩個地方各一份：`configuration_issues`
（Studio，跑之前）與 `locate_crossings` 的 `placement_needs`（CLI 也要有）。

##### 相容

`directions` 預設 `both` → 舊 recipe 沒有這個鍵，拿到的是預設，行為逐位元組不變
（鐵則 9）。三組黃金值逐項相同。section 2／3 那十二格參數用
`show_when=("directions", …)` 跟著方向消失 —— 整組隱藏時小標題也一起不見
（`ParamForm` 本來就會）。

#### 3.3.12 兩件小事：靠邊的框、疊框分色（✅ 2026-08-18）

##### (1) 「靠邊 n px 內的框自動拿掉」—— 兩張卡共用的勾選框

> 「Profile 跟 Template 都幫我 gen 一個 checkbox（可勾選要不要使用的），
>   功能是靠近邊界 n pixel 內的 ROI box 會被自動拿掉。」

一個框壓在 patch 邊上，量到的是**半截的**那一塊：像素少，而且少掉的是同一側的
那一半。它照樣吐得出一個看起來完全正常的灰階值，然後混進 `<name>_others` 那個
基準裡。（Template 這邊實測得很直接：一張 137 px 寬的 patch 上鋪出四份，最右邊
那一份寬度是 14 而不是 16。）

`drop_edge`（勾選）＋ `edge_margin`（px，`show_when` 跟著出現），**兩張卡同一組
參數名**，`_util.drop_edge_specs()` 是唯一的那一份 —— 使用者心裡這是一件事，
換一種定位法不該重學一次。為什麼不是「`edge_margin = 0` 代表關掉」：使用者要的
是一顆**看得到的勾選框**，用 0 當哨兵的話「這張 recipe 有沒有在做這件事」要靠
看一個數字是不是 0 來推，而那個推論在畫面上不存在。

**三個會安靜出錯的地方，全部處理掉了：**

1. **滿版的那一軸不算「靠邊」**（量出來的，不是想出來的）。單方向的 Profile
   每一個框都是滿版的（y=0、高 = 整張圖），照「碰到邊界就算」的話**每一個框都會
   被丟掉**，只剩豁免的中心那一塊 —— 實測 6 個變 1 個，而畫面上沒有任何錯誤訊息。
   滿版不是「放在邊上」，是「這一軸整個都要」。所以某一軸上框跟影像一樣長時，
   那一軸不判定。
2. **缺陷所在的那一塊永遠留著。** `<name>_center` 不是母體裡的一個樣本，它是
   **被量的那個東西**（patch 以缺陷為中心裁切）。丟掉它的話 `_center` 會安靜地
   指到另一塊 —— 那一塊裡沒有缺陷，而下游每一個數字都照樣算得出來。而且濾要在
   **挑出中心之後**：順序反過來，中心會從「離缺陷最近的那一塊」變成「留下來的
   裡面離缺陷最近的那一塊」。
3. **基準被濾光時要說對原因。** `set_region_family` 原本那句是「這張 patch 只有
   一份」；被濾光的時候那是錯的，而兩者的處置相反（前者換張圖、後者改一個數字）。
   多一個 `edge_dropped` 參數只影響那句話。

每個區域多一個特徵 `<name>_edge_dropped`（Profile 是 `cross_edge_dropped`）。
它要在，是因為這個開關**會安靜地改變基準的樣本數**：同一份 recipe 在 wafer 中心
與邊緣的 defect 上留下的框數不一樣，而 `_others` 的統計本來就跟樣本數有關。

`drop_edge` 預設 `False` → 舊 recipe 行為逐位元組不變（鐵則 9），三組黃金值逐項
相同。`edge_margin` 的 max 是 **64**（不是 1000）—— 有上下界才有滑桿，而上界設在
一個沒有人會用到的地方等於把滑桿變成裝飾。

##### (2) 疊框一個區域一個顏色

> 「Image Stream 顯示上 顏色 overlay 重疊會同個顏色（藍色）」

Region-1 之後**一張卡可以標好幾個區域**，而 `StudioWindow.region_overlay()` 把
它們全部攤平成一串框、`ImageView._paint_overlay` 全部畫成 accent 藍。兩個區域疊
在一起時畫面上就只是一團藍線 —— 而使用者要判斷的正是「哪一塊是 ROI1、哪一塊是
ROI2」。

* 顏色搬到 `theme.REGION_COLORS`（原本住在 `cell_canvas`），因為它有**兩個**使用
  者：模板編輯器的畫布與 patch 上的疊框。使用者在對話框裡把 ROI1 畫成綠色的，
  到了 patch 上它就要還是綠色的 —— 顏色不同的話，畫面上沒有任何東西說得出那兩個
  是同一個東西。`region_check` 的縮圖也換成同一組（它的註解本來就寫著「三個一樣
  的藍框排在一起，看不出哪個是主角」）。
* **顏色 = 哪一個區域，線寬與角標 = 哪一塊是缺陷那一塊。** 焦點框原本畫成**紅色**
  ——分色之後不能再那樣，紅色會被讀成第三個區域。改成同色加粗＋四個角標：那是純
  幾何的記號，跟任何區域顏色都不衝突，而且在框只剩幾個像素、線寬看不出差別時仍
  然看得見。（同一條原則在 Region-2b 的分群色帶上用過：兩個問題，兩個位置。）
* 兩個以上的區域才畫**左上角的圖例**。只有一個的時候那個顏色沒有在跟誰對比，
  一行字只是擋住影像。
* 框與名字走**同一個清單**（`_overlay_region_names`，從 `region_overlay` 抽出來
  的）。兩份各自算的話，區域一多顏色就會指到隔壁那個 —— 而畫面上沒有任何東西透露
  這件事。長度對不上時 `set_overlay` 整組不分色：**錯位的顏色比沒有顏色糟得多。**

使用者中途要過「著色填滿（可以透明點）」又收回（「更正 不要著色填滿好了」），
所以維持只畫外框。

#### 3.3.13 Region-3 定調成 **RSEM only**，並先寫一支健檢（2026-08-18）

##### 使用者定調

> 「for GDS 這塊，我暫且不打算做 patch 的部分⋯⋯當初 GLAS 在設計時是瞄著 RSEM
>   image 大圖去對的，因為 SEM image 清楚 pattern 多（**GDS 使用時機通常會在
>   非週期性重複區域**）。但如果是 for patch —— 先不說在 GLAS 那邊 patch 的
>   module 沒打通，光是 patch 要怎麼跟 GDS layout 對位就會很頭大（patch 小，
>   可供對位的 layout 小）。我們產的那些 png mask，都是在**已經對位完 no shift**
>   情況下產生的。」

##### 這一刀砍掉的比看起來多

| 原本的阻礙（§4 給 GLAS 的需求）| RSEM only 之後 |
|---|---|
| 必要 1：多頁 TIFF 的 page 對應 | **不需要** —— 一顆一個檔，1:1 |
...
卡片吃的是**一條流**，它不看 `dataset.kind`。所以「先做 RSEM」不需要任何開關
（`scope.py` 一個字都不加），也不會擋住以後：patch 那條路要打通的是 GLAS 端與
ingest 的配對，卡片一行都不用改。

##### 但「非週期」改變了一個設計

Template 要有週期、Profile 要有條紋，**GDS 就是兩者都沒有時的那條路**。兩個推論：

1. **這張卡不准借另外兩張的任何東西** —— 不做週期估測、不找條紋。那兩套工具就在
   隔壁，很容易順手拿來補洞。
2. **這張卡只吐 `<name>`，不吐 `_center` / `_others`。** 另外兩張卡的 `_center`
   是「缺陷所在的那一份」，而那是**幾何保證**的（patch 以缺陷為中心裁切、那些
   形狀是同一個重複結構的複本）。這條路上兩個前提都不成立。硬給的話使用者會照
   Template 的直覺寫 `epi_center - epi_others`，在一張卡上有意義、在另一張上是
   噪音，**而畫面上不會說**。方向是不對稱的：**之後加一個區域名是免費的，
   之後拿掉一個會弄壞已經寫好的 recipe** —— 所以從窄的開始。
   （使用者：「缺陷在影像中心其實沒差，我最主要是想把 ROI 在 non-repeating
   pattern 給框出來，沒有沒關係之後再說。」）
...
`_label_view.png`／`_gray.png` 不能拿來切區域、CSV 的編碼不安全（JSON 才是）。
...
| 2 | ingest 掛 sidecar（`layout_label`）+ 載入卡宣告那條流 | ✅ 2026-08-18 |
| 3 | `roi_from_mask`：連通元件 → 精確拆成矩形 → 具名區域 | ✅ 2026-08-18 |
| 4 | Studio：掛匯出的入口、`layers` 的表格編輯器、儀表 | ✅ 2026-08-18 |

第 1 步「刻意非週期」是有理由的：只用條紋測的話，偷偷跑進去的週期假設永遠不會
被抓到。
...
#### 3.3.15 第 2 步：sidecar 進 ingest、`load_sidecar` 卡（✅ 2026-08-18）

三個檔：`core/ingest/glas_export.py`（配對）、`DefectItem.sidecars`（住哪裡）、
`core/steps/load_sidecar.py`（載成一條流）。CLI 多一個 `--gds <匯出資料夾>`。

##### 附加檔**不能**放進 `DefectItem.images`

`load_single` 的契約是「一顆一張」，而且它在一顆有兩張時**拒絕載入**（那個拒絕
是對的 —— 見 `steps/load.py`）。label 混進 `images` 的話，每一顆 RSEM defect 都
會突然變成「兩張」而載不進來，而錯誤訊息會**說謊**（「這顆有 2 張影像」；不，
它有 1 張影像跟 1 個附加檔）。所以有 `DefectItem.sidecars`，而讀它的是自己一張卡。

順帶：`load_sidecar` 在畫布上是一個**來源**（沒有輸入埠）——
它的輸入不是影像流，是 ingest 掛上來的檔案。

##### 兩個實測抓到的、會安靜出錯的地方

**(1) 換一份匯出，快取不會失效。** 有 KLARF 的時候 `_dataset_token_for` **只看
KLARF 的 stat**，而換 mask 目錄完全不會動到 KLARF。於是同一個 lot 換一份匯出，
token 一模一樣，影像段快取把**上一份匯出算出來的框**餵回來 —— 跑得完、有數字、
而且是錯的（鐵則 9 講的正是這個形狀）。修法是把 sidecar 的路徑 + mtime + size
折進 token；**一顆都沒有 sidecar 時回空字串，所以既有的 token 逐字元不變**
（既有快取與黃金值不受影響）。

**(2) `load_raw` 會把三通道的圖 `cvtColor` 成灰階 —— 安靜地。** 對 SEM 影像那是
對的，對 label map 是致命的：像素值**是**層號，加權平均會把 1、2、3 混成不存在
的值。而 GLAS 匯出時 `<id>_label.png` 旁邊就放著三通道的 `<id>_label_view.png`。

實測（同一個檔）：

```
load_raw   → [0, 131, 159, 182]     ← id 被混掉了，沒有任何錯誤
load_exact → ndim 3, (256, 256, 3)  ← 看得見它是三通道
```

**這一條特別值得記**：我原本在卡片裡寫了「`ndim != 2` 就報錯」，而那段是**死的**
—— 通道早在讀檔那一層就被合掉了。新增 `imageio.load_exact()`（原樣讀，不合通道），
附加檔走它，通道數的判斷留給卡片講（而它現在講得出「那多半是 `_label_view.png`」）。

##### 掛不上的那幾顆不是錯誤

GLAS 的分數門檻本來就會擋掉一些（那一列的 `label_png` 是空的）。那幾顆跑到這張卡
會失敗、講出**兩種可能的原因**（沒掛匯出 vs 這一顆被擋掉），而整批照跑（鐵則 7）。
實測 8 顆的 lot：7 成功 1 失敗，訊息是可以照做的。

##### 還沒做的

Studio 那一頭還沒有「掛上匯出」的入口（CLI 有 `--gds`）。它跟第 4 步的儀表一起做。

#### 3.3.16 第 3 步：`roi_from_mask`（✅ 2026-08-18）

`algo/mask.py`（幾何）＋ `steps/roi_from_mask.py`（卡片）。端到端跑過：
`load_single` → `load_sidecar` → `roi_from_mask` → `glv_stats`，`workers=2`，
分數表達式吃 `L17_D0_glv_mean`，CSV 出得來 —— **下游一行都沒改**。

##### 唯一不能錯的東西：框聯集起來就是那一層

一個 L 形的 bounding box 會**框到別的材質**，而它照樣吐得出一個看起來完全正常
的灰階值。站點的 layout 是 Manhattan 的，所以精確拆解是**等價不是近似**。
測試斷言的是「聯集**逐像素**等於 `label == id`」，而且是**正規化來回之後** ——
0–1 座標是為了換尺寸不錯位（F7-4 的坑），但它同時是一個會偷偷四捨五入掉一個
像素的地方。實測 5 組隨機 layout × 2 層、以及合成匯出的 3 顆 × 3 層：差 0 個像素。

##### `max_boxes` 的數字是量出來的

使用者：「max boxes 理想上應該會很多，原因是處理的會是 **RSEM images**
（不是之前的 patch）等級。」對，而「很多」是多少（1000×1000）：

| 形狀 | pieces | rectangles |
|---|---|---|
| 密集 line/space | 39 | 39 |
| 同上，被另一層橫向切碎 | 975 | 975 |
| 45° 斜帶（`fillPoly` 沒有 LINE_AA，斜邊就是 1px 階梯）| 10 | **5 295** |
| contact 陣列 6×6/14 | 5 184 | 5 184 |

下游成本（**每張量測卡每顆各一次**）：N=1 000 約 3 ms、N=10 000 約 60 ms、
N=50 000 約 280 ms、N=200 000 約 1 s。

→ 預設 **8192**（蓋得住實測最壞的 5 295 還有餘裕）、上限 **65536**
（再上去就是 400 顆 × 幾張卡用分鐘算）。既有 Region 卡的 64 在這條路上會
**安靜地砍掉 95%**。砍到上限一定要講：`<name>_clipped` ＋ 一句警告。

拆解本身 31 ms（1000×1000 最壞情形，numpy 向量化取 run + run 層級的 union-find）。
另有一條 `MAX_RUNS = 200 000` 的硬線：棋盤格那種病態輸入會產生幾十萬個 run，
而「跑很久」跟「當掉」對使用者是同一件事 —— 擋下來並講出「這多半不是 label map」。

##### layer 名的改寫

真實匯出的三個 layer 名（`L17/D0` 那種形式）**都不能當變數名**，而區域名會變成
特徵前綴。`glas_export.region_name_for()` 給一個**每次都一樣**的預設
（`L17/D0` → `L17_D0`，撞名補 `_2`），而使用者可以改名 —— 跟 F11 Input 的通道
命名同一個原則：程式給預設，名字是使用者的（`epi_glv_mean` 比
`L17_D0_glv_mean` 好用太多，而寫分數表達式的是他）。

`layers` 參數直接**重用 `channel_map` 型別**：兩者都是「整數 → 名字」，而那一份
的驗證規則（≥1、不重複、名字要能當變數）正好就是這裡要的 —— 不發明第二個解析器。

##### 順手做掉的：Studio 的「Open GDS export…」

本來排在第 4 步，但**必須提前**：這張卡的「還沒設定完」那句話要指向一個按得
下去的東西（`tests/test_ui_f7_9_feedback.py` 有一條不變量在擋 —— 指向不存在的
東西，那句話本身就是死路）。所以工具列多一顆：問資料夾 → `glas_export.attach`
→ 把結果講在狀態列 → **把 layer 名填進每一張還沒設定的 `roi_from_mask` 卡**
（對照表就在 manifest 裡，讓使用者自己抄一次是在製造一個可以抄錯的機會）。

沒有資料集時那顆鈕是灰的 —— 它不是第五種 source，是往已經載好的每一顆上再掛
一個檔案。已經改過名字的卡**不會**被重新掛載洗掉。

#### 3.3.17 第 4 步：Studio 那一頭（✅ 2026-08-18）

##### `layers` 的表格：一個旗標，不是第二個 widget

`ChannelMapField` 原本是照「一列一張圖、左欄是頁碼」設計的。label id 不是頁碼 ——
但**資料形狀完全一樣**（整數 → 名字，空的就是不要），差的只有三句話。所以是
`ParamSpec.row_kind`（`"images"` / `"labels"`）一個旗標，不是抄第二個 widget
（抄第二份出來的那份一定會漂移，這個 repo 記過三次）。

換掉的三句話：左欄的「Image 3」→「Layer 3」、加一列那顆鈕、以及 **placeholder**
—— 那一句最重要：`load_patch` 空著是「用預設名（test / ref）」，這裡空著是
**「這一層不要」**，兩者差很多，所以 placeholder 講的是後果不是一個假名字。

列數各排各的：`set_image_count` 與 `set_label_count` 是兩個數字，而且**互不覆蓋**
—— 一張 recipe 上可能同時有 `load_patch` 與 `roi_from_mask` 兩個 `channel_map`，
用同一個數字排兩者，其中一邊一定是錯的（有測試鎖）。

##### 儀表：**一張表，不是 label 的上色預覽**

第一版想畫上色的 label（label map 幾乎全黑，那正是 GLAS 另外產
`_label_view.png` 的原因）。但**形狀已經看得到了**：這張卡吐的每一層都是一個
具名區域，而預覽影像上的疊框本來就一個區域一個顏色、還帶圖例（第八輪做的），
而且是畫在**真的那張 SEM 影像**上 —— 比另外看一張示意圖有用。

所以儀表回答那張圖答不出來的三件事：**哪一層根本沒落在這一顆上**（框看不到 =
可能是沒有、也可能是被別的層蓋掉）、**各幾塊幾個框**（切碎的程度）、
**有沒有砍到上限**。外加一條：**在 label 圖裡、但 recipe 沒給名字的 id** ——
那是「匯出多了一層而 recipe 沒跟上」，它會安靜地少一個區域。

顏色與順序跟疊框、模板編輯器同一組 —— 表上第二列的顏色就是畫面上第二個區域的
顏色。

##### 一個自己踩的（值得記）

這幾條 UI 測試第一版接在 `tests/test_glas_sidecar.py` 後面，結果**核心那一輪
（不含 UI）整個行程崩掉** —— 那一份不叫 `test_ui_*`，於是 Qt 被拉進「一個行程
...
#### 3.5.1 `roi_compare` —— 把「拿哪兩塊比」變成看得見的一張卡（✅ 2026-08-18）

**這張卡不解鎖任何功能。** 今天沒有它也比得出來 —— 只是比較發生在**分數表達式**
裡（`test_epi_glv_mean - ref_epi_glv_mean`）。而表達式裡的減法：畫布上看不到、
recipe 的 diff 讀不懂、兩邊挑錯區域也沒有人擋得住。它的價值是那三件事變成可見的。

##### 為什麼是**兩對**（流 + 區域），不是一條流兩個區域

使用者列的三種情況（§3.3.6）裡，中間那一種**兩邊的流不一樣**：

| 情況 | target | reference |
|---|---|---|
| patch，跟自己兩側的 EPI 比 | `epi_center` @ test | `epi_others` @ test |
| patch，跟 ref 那張的同一塊比 | `epi` @ **test** | `epi` @ **ref** |
| 單張 source | `epi_center` @ single | `epi_others` @ single |

一條流配兩個區域表達不出第二種。所以是**兩個輸入埠**，而畫布上那就是兩條線 ——
正好是 F9 的規矩（一個輸入埠一條線）。三種情況用同一張卡說得完。

##### 它撞到一條既有的不變量，而那條不變量需要一個例外

`test_every_measure_card_can_take_more_than_one_source`：每一張量測卡都要是
`MultiSourceStep`（`source` 是 `image_keys` 清單）。這張卡不是 —— 而它**不該是**：

「多連一」講的是**同一件事做在好幾條流上**（量 test 也量 diff）。這張卡的兩條流
有**自己的角色**，而角色排不成一串：清單答不出「哪一條是 target」。

所以測試多了一個 `ROLE_PORTS`，而且**驗的東西反過來**：那一類要有**兩個以上的
單一來源埠**，而且不准混用清單型別（兩種接線方式對不起來）。列名字而不是放寬
條件 —— 加第 18 張量測卡的那天，它仍然要做一個明確的決定。

##### 順手把另一條不變量修對了

`not-configured` 的訊息「必須指向一個按得下去的東西（`…` 結尾）」。這張卡缺的
不是一份要匯入的檔案，是**兩格要挑的值**，而那兩格就在旁邊 —— 沒有鈕可以指。

所以那條不變量現在認**兩種**形狀（鈕 **或** “引號”起來的欄位名），而且引號那一種
**要驗**：引號裡的字必須真的是這張卡的欄位名或真的有那顆鈕，不然「指向一個東西」
會退化成「寫一句看起來像樣的話」。第一版我把舊規則**換掉**而不是加上去，結果
`roi_mask` 那句本來講得很好的訊息突然變成違規 —— 用「或」不是「改成」。

##### 算什麼

`delta` / `ratio` / `percent` / `snr` / `tstat`（`algo/glv.compare_pixels`）。

`snr` 是 `(μ_T − μ_R) / σ_R` —— **這個 repo 已經有兩個地方在用它**
（`algo.glv.group_snr`、`algo/snr`），所以不發明第三種寫法：同一個名字在不同
卡片上算出不同的東西，是最難發現的那種錯。`tstat` 是它的「樣本數也算」版本
（Welch），因為 GDS 的一層可能是另一層的十倍大 —— 兩個問題，兩個數字。

分母是 0 的時候是 **nan 不是 0**：0 的意思是「沒有差異」，而這裡的事實是
「這個問題答不出來」。

##### 兩個擋在跑之前的

* **同一塊比自己**（同區域同流）→ 每個數字恆為 0，而那些 0 不會因為任何缺陷而
  改變。跑得完、有數字、而且那些數字什麼都沒說。
* **區域不在**時講出**真正的原因**：`<name>_others` 在只有一份的 patch 上是
  **不存在**的（不是空的），那不是設定錯。實測 8 顆的 lot 上兩種失敗各講各的話：
  「這一顆的 label 裡沒有第 2 層」vs「這一顆根本沒有 label」。
