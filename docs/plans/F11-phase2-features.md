...
##### 要你確認的四個格式問題（這些決定要不要做上面那些）

1. **patch 的位元深度是 8 還是 16（或 12-in-16）？** —— 最重要的一題，見上面。
2. **五通道是在同一個大 TIFF 裡**（跟現在的 patch 一樣，每顆佔 5 頁），
   **還是一顆一個 TIFF**（5 頁）？前者現在就吃得下、後者是接頭 B。
3. **TIFF 的壓縮方式**（none／LZW／deflate／JPEG）？`tifffile` 大多吃得下，
   但舊式 JPEG-in-TIFF 與 packbits 偶爾會出事，知道了就能先擋。
4. **每顆的影像尺寸是否相同**？（不同尺寸現在跑得動 —— 座標都是正規化的 ——
   但 Golden Cell 與 template 那條路會受影響。）

前三題都可以用一支 `fab_probe/` 腳本一次回答（stdlib-only 單檔、純文字輸出、
預設遮蔽識別碼、不需要搬整個 repo）。要我寫嗎？
