...
##### A. 把核心大小畫在影像上（`flatten` 的 `size`、`denoise` 的 `ksize`）★
...
- **機制而非個案**：`ParamSpec` 已經有 `unit`（`flatten.size` 就是 `unit="px"`）。
  規則可以是「影像段的卡片上，`type="int"` 且 `unit="px"` 的參數 → 畫一個那麼大的
  框」。這樣以後任何一張卡填了 `unit="px"` 就自動有，零 per-card 程式碼。
- 成本：`ImageView` 多一個 overlay（已經有量測尺與 ROI 框兩個先例）＋ Studio 在
  選取節點／改參數時餵值進去。

##### H. 這一批有幾顆被削平（`clip_frac` 的整批分布）★
...
成本。成本：`CurveField` 收一組 counts；Studio 在選取 tone 卡時餵預覽影像的直方圖。

##### E. 磨掉的是雜訊還是訊號（`denoise` 的 bilateral / nlm）

一個數字：`RMS(before − after) / σ`。≈1 表示磨掉的量級就是雜訊；≫1 表示連結構
一起磨掉了。`after_stream` hook（⑥ 已經加了）拿得到前後兩張圖，σ 也已經量了 ——
所以這是**五行**的事，而它回答的正是 `strength` 那個旋鈕唯一的問題。

##### B. 兩條流現在真的可比了嗎（`normalize`）

面板已經並排畫兩條流的直方圖，但**沒有一個數字說它們有多像**。可比性是
`subtract` 的前提，而「不可比」在畫面上長得像「兩張圖本來就不一樣」。
提案：處理兩條以上流的 Enhance 卡多報 `pair_level_delta` / `pair_spread_ratio`。
⚠ 這一項會**改動既有 recipe 的特徵集**（dual-route 的 recipe 有同時處理兩條流的
Enhance 卡），所以要單獨一輪並重凍黃金值。
