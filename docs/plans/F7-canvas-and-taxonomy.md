...
* **`Spread` 面板沒有橫軸。** 三排長條右邊各給一個數字，看不出橫軸範圍，
  也沒說紅色直線是「這一顆站在哪」。F7-21 幫主直方圖補了軸與圖例，
  這個量測卡儀表沒跟上 —— 同一種東西兩套做法，正是 §27.7 在清的那種。
* **隱含順序的虛線繞遠路。** 從第一列最右端繞回第二列最左端，而且繞到比第一
  張卡還左邊。三列就三條長折線橫過整個畫布，比它要表達的「順序」還搶眼。
