...
`roi_template` 的比對曲面與三道閘門、`blob_segment` 的 label map、
`subtract` 的背景殘差、`roi_snr` 的分子分母分解、Region 卡的「框畫在前 N 顆縮圖
上」常駐帶狀圖，以及所有卡片共用的頁尾「這張卡在這一批花了多少時間」
（`traces` 裡本來就有）。`roi_profile` 的曲線面板也應該收進這個機制 ——
現在它是另一條路，兩條並存會腐爛。
