...
**「同樣的參數」不等於「同樣的變換」。** 這是整個 F7-19 唯一會出錯的地方。

| 類 | 卡 | 一張卡吃 N 條流？ |
|---|---|---|
| **A. 查表／固定核**（參數完全決定變換） | `brightness_contrast`、`gamma`、`invert`、`denoise`、`flatten` | ✅ 直接 N 進 N 出，語意乾淨 |
| **B. 從影像自己量出來的**（參數一樣，變換不一樣） | `percentile_norm`、`glv_mask_norm`、`local_contrast` | ⚠️ 必須多一個「在哪一條流上量」 |
| **C. 本來就多進少出**（具名角色） | `hist_match`（moving+reference→moving）、`subtract`（a+b→diff） | 已經是對的，不動 |

B 類的失敗長這樣：`percentile_norm` 是「量出這張圖的 P2–P98，拉到 0–255」。
同一張卡對 test 與 ref **各自量各自的範圍**，兩張圖就被拉到不同的尺度，
然後拿去相減。跑得完、有數字、而且是錯的 —— 這正是 F7-18 發明 `range_from`
（§22.5）的原因，那個理由在新形狀下**一字不改地成立**。

### 23.4 B 類：`range_from` 變成 `measure_on`，順序陷阱自己消失

B 類卡多一個參數：

```
measure_on:  each（每條各自量，預設）| test | ref | <任一條接進來的流>
```

這比現在**簡單**，而且比現在**安全**：

- 現在是 `range_from`（填另一條流的名字）**加上**「這張卡要排在會改寫那條流的
  卡前面」—— 後者只寫在 help 文字裡，沒有任何東西擋它（§22.7 第三條）。
- 新形狀下，量與套用發生在**同一張卡的同一次執行裡**：先量 `measure_on` 那條流
  的原始值，再套到每一條。中間沒有別的卡插得進來，**順序陷阱不存在了**。

`local_contrast`（CLAHE）是 B 類裡的異類：它是逐 tile 自適應的，沒有一個
「範圍」可以借。詳見 23.9。
...
|---|---|---|
| 誰 | A/B 類：Brightness、Denoise、Percentile… | C 類：`subtract` 的 `a`/`b`、`hist_match` 的 `moving`/`reference` |
...
⚠️ 這是一個**語意反轉**，不是新增功能。C 類卡（角色埠）必須維持取代語意 ——
...
寫回同名的流。B 類卡在迴圈**之前**先算好 `measure_on` 那條的基準值。

`resolve_reads` / `resolve_writes` 回傳整份 `streams`（加上 B 類的 `measure_on`）。
...
- 舊：`{"source": "test", "range_from": "ref"}` → `{"streams": "test", "measure_on": "ref"}`。
...
1. **`local_contrast`（CLAHE）算 B 類還是 A 類？** 它是逐 tile 自適應，沒有單一
   「範圍」可以借，所以 `measure_on` 對它沒有意義。同一張卡套到 test 與 ref 上，
   兩者拿到的**確實是不同的變換** —— 這在「增強後給人看」是可接受的，在
   「增強後拿去相減」是危險的。三個選項：歸 A 類並在 help 講清楚／不准它吃多流／
   給它一個「用哪條流算 tile 的直方圖」（技術上做得到但很難跟使用者解釋）。
   **傾向第一個**，但要在卡片說明裡明講「這張卡會讓兩條流變得不完全可比」。
2. **輸出埠要不要在「這張卡只是原地改寫」時仍然畫 N 個點？** 畫 N 個點誠實
   （進什麼出什麼），但一條 9 張卡的鏈上會有 18 個點。傾向畫，因為它就是真相。
3. **角色埠與流埠在視覺上怎麼分？** 標籤來源不同已經是一個區別，但可能不夠 ——
...
`cd_gate` 同樣相同。B 類卡另外要單獨驗一次「`measure_on` 的結果 == 舊的
`range_from` 排對順序的結果」—— 那是這一輪唯一真的改變了數值路徑的地方。
