...
> （描述卡片**在解決什麼問題**）。
>
> 版本：v0.1 · 2026-07-28 · 依據 M7 使用者回饋（UI/UX 檢討三輪）
...
| 階段 | 內容 | 風險 | 為什麼是這個順序 |
|---|---|---|---|
| **F7-1** | patch-only 收斂（UI 層關掉 rsem、藏兩張卡、範本只留 patch） | 低 | 先把世界變簡單，後面每一步都少一半情況要想 |
| **F7-2** | 主題換色 + 平面化 | 低 | 早做，之後判斷版面才不會被暖色帶偏 |
| **F7-3** | 卡片分類重整（`Step.group` + 搜尋 + 前置條件 badge） | 中 | 不動引擎 |
| **F7-4** | Region 段 + 量測卡遷移（破壞性 schema） | 中高 | 要同時改範例 recipe 與測試 |
| **F7-5** | Results 視窗（直方圖 + Gallery 搬家） | 中 | 把主視窗空間讓出來 |
| **F7-6** | 畫布 | 高 | 最大一塊，等前面把地基整乾淨 |
