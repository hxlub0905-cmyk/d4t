...
## 14. F15-4：只在該找的地方找（2026-08-20）

使用者把最關鍵的一句話講出來了：

> RSEM 基本上就是移到 KLARF 座標（defect 在中間），不置中的原因是**機台 stage
> 移動的 offset**（無法避免）……可以抓 FOV ~15% 左右。

那句話把「大海撈針」變成「量一個小偏移」：

```
RSEM 大圖（機台先移到那一顆的座標才拍）
┌──────────────────────────────────┐
│            ┌───────┐             │   ○ = 圖中心 = KLARF 說的那一點
│            │  ○ ●  │  ← 只在這個 │   ● = defect 真的在哪
│            └───────┘     框裡找   │   兩者的差 = stage offset
│              ±15% FOV            │
└──────────────────────────────────┘
```
...
| 參數 | 預設 | 做什麼 |
|---|---|---|
| `search_within` | **15 % of FOV** | 只在這個框裡找。0 = 整張圖（這張卡單獨使用時的樣子）|
| `expect_dx_px` / `expect_dy_px` | 0（進階）| 框的中心要不要移一點 —— 系統性的那一份 stage offset |

| 特徵 | 意思 |
|---|---|
| `align_off_x_px` / `align_off_y_px` | **偏離預期位置多少 = 這一顆的 stage offset** |
| `align_peak_ratio` | 第一名 ÷ 第二名的峰值（0…1）|
| `align_dx_px` / `align_dy_px` | 模板左上角在大圖裡的絕對位置（查問題用）|

### 14.2 為什麼要第二名（`align_peak_ratio`）

陣列區（記憶體、週期性 layout）裡，同一個模板在**好幾個地方**都拿得到一樣高的
分數 —— 而「峰值 0.99」這個數字本身完全看不出這件事。實測：

| | 峰值 | peak ratio |
|---|---|---|
| 隨機紋理（真的對到） | 1.00 | **0.05** |
| 20×20 週期性圖案 | 1.00 | **1.00** |

兩者的峰值一模一樣，而右邊那一列的位置是**猜的** —— 換一顆雜訊就跳到隔壁去。
這正是「跑得完、有數字、而且是錯的」在對圖上的長相。

### 14.3 校正迴圈（stage offset 是系統性的）

1. 跑一批，看儀表上 `align_off_x_px` / `align_off_y_px` 的分布；
2. 取**中位數** → 這台機／這批的 stage offset；
3. 填進 `expect_dx_px` / `expect_dy_px` → 框跟著移，於是**框可以縮小**；
4. 而**每一顆偏離中位數多少**本身就是「這一顆配對可不可信」的指標。

實測：偏 150 px（37% FOV）的那一顆，15% 的框抓不到（而且錯誤訊息會講出框的
存在，不會謊稱「這兩張不像」）；把 150 填進 `expect_dx_px` 之後，**5% 的框**
就抓得到，而且 `align_off_x_px` 回到 0。

### 14.4 明講沒做的：用兩份 KLARF 的座標差當錨點

`pair_source` 手上有兩邊的座標，所以理論上可以把「預期位置」算得更準
（中心 ＋ 座標差 ÷ nm/px）。**沒做**，因為 KLARF 的 y 軸與影像的列方向誰正誰負
還沒有在真實資料上驗過 —— 猜錯的話框會往反方向移，而那會把對的那一塊排除掉。

先讓 `align_off_*` 把它**量出來**：整批的中位數如果不是 0，那就是這個偏移，
而它的正負號會直接寫在數字上。量過再做，跟 GLAS 那條路一樣。


---

## 15. 怎麼驗證（`tools/pair_probe.py`，2026-08-20）
...
| | 問題 | 報告上的哪一列 |
|---|---|---|
| ① | 容差要設多少 | `match_dist_nm` 的 p90 |
| ② | 兩邊的 nm/px 比例對不對 | `ncc_score` 的中位數 |
| ③ | stage offset 是多少（**以及 y 軸的正負號**）| `align_off_x/y_px` 的中位數 |
| ④ | FOV 的 15% 夠不夠 | 扣掉中位數之後的散布 |

所以 `tools/pair_probe.py` 跑一批、印分布、**直接講出建議值**。輸出跟
`check_glas_export.py` / `load_probe.py` 同一條路：預設不印任何廠內識別碼，
可以直接貼出來。

```
python tools/pair_probe.py 主.001 第二份.001 --n 200 --carry PMSCORE,DSIZE
```

### 15.1 工具自己要先是對的

報告上寫「stage offset 中位數 (12, −7)」的時候，那必須真的是 12 與 −7，
不是一個看起來很合理的數字。`tests/test_pair_probe.py` 自己造一份「大圖裡真的
藏著主 lot 那一塊 patch」的第二份 lot（位置是我們放的），然後驗報告有沒有把它
讀出來 —— 含**尺度錯的那一種**：報告要說「那通常不是配錯顆，是 nm/px 比例
錯了」，因為前一句話會讓人往完全錯的方向查。

合成資料證明不了真實資料會怎樣；它證明的是**這支工具沒有在說謊**。

### 15.2 讀報告的順序

**由上往下，前面的沒過就不要看後面的** —— 它們是串起來的：

1. **配對率** < 50% → 停在這裡。容差太小，或兩份 KLARF 的座標系根本不同
   （把 `--tol-nm` 開大 10 倍，配對率跳上來就是容差問題）。
2. **`ncc_score` 中位數** < 0.3 → 尺度或搜尋框的問題，不是配對的問題。
   先 `--within 0`（不限制搜尋框）再跑一次：分數上來 = 框太小；沒上來 = nm/px。
3. **`align_off_*` 中位數** → 這就是 stage offset。填回 `expect_dx_px` /
   `expect_dy_px`。
4. **扣掉中位數之後的散布** → 這才是 `search_within` 真正需要多大。
5. **`peak_ratio`** > 0.9 的比例 → 週期性結構有多嚴重。超過兩成的話，
   patch 要開大一點（帶到更多不重複的結構），或把它寫進分數表達式當門檻。
