...
## 8.7 H2H 的 UI —— 預覽上的標記與專屬面板

使用者 2026-08-26：「H2H 的 UI 可以更有趣、更人性化那樣嗎（像 GLV CD 那樣）？
有圖示？Image stream 會提供預覽上方的 overlay 嗎？」

### 先回答：overlay 的機制**本來就有**，而且跟著 Image stream 走

`Step.overlay_marks(ctx, params, stream)` 是 F19 建的、**整個 Measure 段共用**
的那條路：Studio 的 `_refresh_measure_marks` 拿「現在選著的那張卡」問它要畫什麼，
`stream` 就是 Image stream 下拉現在的值（比對模式下兩個 view 各問各的）。

問題是**只有 CD 與 GLV 實作它**。H2H 什麼都不畫 —— 而它做的事偏偏是「位置」。
這一輪補上：

* **框** —— 小圖真的對到的那一塊；
* **十字** —— 機台瞄準的那一點。

跟報表上那兩個記號是**同一組數字的兩種說法**（§8.6）：報表是跑完之後的證據，
預覽是**調參數的當下** —— 拖 `search_within`、填 `expect_dx_px` 的時候就看得到
框往哪裡跑，而不是跑完一整批才知道。

⚠ **只畫在被搜的那條流上**（`meta["align_to"]["search"]`）。座標是**大圖**的，
畫到 `paired`（模板）或 `aligned`（裁出來的那一塊）上就指著錯的地方 ——
而正規化座標會讓它看起來像個正常的框。實測：`single` 6 條、`paired` 0 條、
`aligned` 0 條。

### 專屬面板（`H2HInspector`）

以前是通用的 `MeasureInspector`（五條分布圖）。分布是調參數要看的，**留著** ——
所以新面板是它的**子類**，只在上面多一行講分布答不出來的三件事：

```
matched at (41, 60)  ·  off by (-23, -4) px  ·  score 0.06 (runner-up 0%)
```

⚠ 那一行**兩個分數一起講**（`ncc_score` 與 `align_peak_ratio`）。理由見 §8.5：
陣列區裡 NCC 0.98 而位置每一顆都錯，只看分數的人會被騙。ratio ≥ 0.9 時那一行
直接寫成「**this position is a guess (repeating pattern)**」。

### 圖示（`icon_choice`）**不做**

CD 的 `shape` / `axis` / `target` 適合圖示，是因為它們是「**幾選一的樣子**」——
一條線 vs 一團東西，畫得出來。H2H 的參數是數字（門檻、百分比、像素偏移）與
影像流，**沒有一格是在選一種樣子**。硬配圖示只會多一排看不懂的方塊。
它缺的不是圖示，是「圖上指出位置」與「一句話講完狀態」，而那兩件這一輪做了。
