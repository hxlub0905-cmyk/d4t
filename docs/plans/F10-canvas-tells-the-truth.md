...
### F10-6：改輸出流的名字，下游跟著走（**稽核抓到的，不是回報的**）

使用者要求「`write result to` 命名成 GGG，畫布上那顆埠也要叫 GGG」——那一半在
F10-2 就做到了。但**下游沒有跟著走**：改名之後下游那張卡的 `source` 還寫著
`diff`、線上的 `src_out` 也還是 `diff`，於是使用者只是幫一條流取個好記的名字，
整條 pipeline 就 `missing-image` 了。而他從畫布上看到的是「線還在」（線是照
節點畫的），所以看不出發生了什麼事。

名字是**顯示用的標籤**，線才是「誰接誰」的事實（見 `Edge.dst_in` 的說明）。
所以改名不該動到接線，只該讓兩端的標籤跟著換：`set_param` 看到被改的是
`direction="out"` 的參數時，把從這張卡出發、`src_out` 等於舊名字的每一條線
改成新名字，並跟著更新下游那一格（`image_keys` 只換掉那一項）。

⚠ 這裡踩到一個陷阱：第一版用 `compound("set-param")` 包住整段，結果**滑桿的
復原合併壞了** —— `compound` 會讓 `_push_undo` 走「一整段只推一次」那條路，
於是繞過 `coalesce`，拖一次滑桿發的幾十次 `set_param` 各記一步。
...
## 5.7 稽核：同一類錯誤還有沒有別的位置

前面五項都是使用者回報的，而它們有**同一個形狀**：某個動作的反向操作只做了
一半（接線改參數但剪線不改、加卡建節點但刪卡不刪線）。所以最後用一支稽核腳本
把每個動作與它的反向操作系統性地走一遍，而不是等下一次回報：

| 稽核項 | 問的問題 | 結果 |
|---|---|---|
| 復原／重做 | 六種動作各做一次再 Ctrl+Z，狀態要逐項回到動作前 | ✅ |
| 改輸出流名字 | 下游的線與參數跟得上嗎 | ❌ → F10-6 |
| 刪中間那張卡 | 上下游各自怎麼辦 | ✅ |
| 改名之後復原 | 動到好幾張卡，但只能是一步 | ✅（修完） |
| 停用一張卡 | 會不會誤報 lint | ✅（沿線往上找還開著的產出者，F9-8） |
| 換順序 | 線與參數不准被動到 | ✅ |
| 載入既有 recipe | 載進來就紅嗎、載完再編輯會弄壞既有的嗎 | ✅ |
| 成環的線 | 被擋下來之後不可以留下痕跡 | ✅ |
| recipe JSON round-trip | 必須是 identity（鐵則） | ✅ |
| 每張卡加→接→剪→刪 | model 最後要乾淨 | ✅ |
| 畫布線數 == model 線數 | 畫布不可以自己多畫或少畫 | ✅ |

十一項裡抓到一個（F10-6），而它正是使用者**還沒踩到**的那一個。
