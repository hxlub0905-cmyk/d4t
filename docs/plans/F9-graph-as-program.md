...
**狀態：Phase 1 契約已定、原型已驗證（2026-08-15）。引擎尚未動工。**
...
不能給 0 分（那是 `cd_x_nm` 那個坑的同一種）。要有一個「沒有結論」的狀態，
而且 Gallery 與輸出都要看得到它。
...
|---|---|---|
| **1** ✅ | 定契約 + 原型證明（**本文件**） | 無 |
| **2** | 引擎換心：依 edges 求值、Packet 取代全域 Context、快取重新定義、舊檔遷移 | **高**，唯一有風險的一段 |
| **3** | 17 張卡搬到新契約；Input 變卡片；ADC 變卡片 | 中（量大但機械，使用者自己寫） |
| **4** | 畫布變成真的編輯器：埠型別擋不合法連線、**刪線=真的斷開**、分岔/合流畫得出來 | 中 |
| **5+** | 開始長功能卡：`CLASSNUMBER` 分流、跨顆統計、ML、更多量測 | 低（純加法） |

Phase 2 的驗收是整件事的安全網：**遷移前後逐顆分數相同**。過不了就不往下走。
