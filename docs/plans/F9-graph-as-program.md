...
**狀態：Phase 1–3b（引擎那一半）完成（2026-08-15）。**
**線決定卡片動哪一條流；判定是卡片。剩下：UI 那一半、輸入變卡片、`routes`/`score` 退場（Phase 3c–4）。**
...
| **3c** | UI 那一半：藏掉流的下拉、埠標名字、兩種線看得出差別；Input 變卡片；`routes` / `score` 退場；`scope.py` 解除隱藏 `adc` | 中 |
