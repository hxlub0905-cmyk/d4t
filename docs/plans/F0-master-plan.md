...
| 形態 | 桌面 GUI（PySide6），core/ui 嚴格分離；**CLI 附帶**（`d4t run <recipe> <klarf>` 供排程/腳本） |
...
| 里程碑 | 內容 | 驗收 |
|---|---|---|
| **M0 抽庫** | Vendoring §5 清單檔案進 `d4t/core`；統一 KLARF parser（KLIP `klarf_core` 為底，補 ImageFileName；棄用重複的 `klarf_parser`）；修三摩擦（ROI 座標統一正規化、SNR 正負號統一、`compute_snr_map` 回傳）；合成影像單元測試 | `pytest` 全綠、core 零 Qt import |
...
4. 命名待定（d4t 為代號；可延續水果/自然系：PEAR、Fusi³…）。
