...
* `tests/test_output_bundle.py`（16 條）＋ `tests/test_decide_tree.py` 多 3 條 ——
  * **C0 搬家**：`decide_tree.py` 的 import 掃過去**沒有 PySide6**；
    UI 那幾個名字 `is` 同一個物件（抄一份出來的話這條會紅，而兩邊的測試
    仍然全綠）；報表與畫面的類別顏色是同一份。
  * **bundle**：四個檔案都在、`images/` 張數 == 顆數、
    **`<img src>` 是相對路徑而且指得到真檔案**（絕對路徑在 `pathlib` 底下
    也會過 `is_file()` —— 那個洞是第一版真的有的，測試因此先問「它是相對的嗎」）。
  * **`limit` 砍的是圖不是報表的列**；`limit=0` 是預設；截斷留下的是分數最高的。
  * **快取**：同一批跑兩次，第二次 `cache.hits == N` 且 `misses == 0`；
    有快取與沒快取寫出來的 JPEG **逐位元組相同**。
  * **大小回歸**：`report.html` < 200 KB、不含 `base64`，而圖的總大小比它大
    （否則「根本沒有圖」也會讓那條永遠是綠的）。
  * **版面**：`What it decided` 排在 `Which ones` 前面、每一類的顆數加起來
    等於這一批、「有卡片出錯」是自己一列。

每一條回歸測試都驗過會紅（把 bug 放回去 → 它抓到）：
拿掉區域偏移、質心改用框算、量不到時寫 0、退路順序寫反、少一格也算框、
改用後綴比對、不排序、偷偷長出區域、面積少乘一次、順序改回字母序、
UI 又抄一份 `display_tree`、`rerun` 不走快取、圖用絕對路徑、
`limit` 砍到報表的列、圖嵌進 HTML、判定段排到最後面、判定段漏掉「算不出來」。
