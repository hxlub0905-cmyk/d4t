...
> 狀態：**議程（等使用者定調，未動工）**。Phase 2 的規矩：預期功能、UI、設定
> 放哪都要先討論過才動手。
...
1. **對不上 map 的預設行為**：走 default route，還是判失敗？（提案：兩種都
   支援，`default` 留空＝失敗）
2. **route 模型**：第一期用選項 A（不同節點 id ＋ drift lint）？
3. **UI**：預覽自動切 route 可以接受嗎（你在看 A 路時點到一顆 B 路的，
   畫布會自己跳）？
4. **`lot_stats` 併這一輪**（第 3 期），還是分開？
