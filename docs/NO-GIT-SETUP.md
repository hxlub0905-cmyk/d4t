...
`bundle/d4t_bundle.py` —— **一次複製就搬完整個 repo**（2026-09-03 是 3.4 MB）。
...
**它是純 ASCII 的**（2026-09-03 起，逐檔 `lzma` + base64）—— 整份檔案裡
一個非 ASCII 字元都沒有，包含最前面約 200 行的解包程式（那一段是英文的）。

> ⚠ **這一點是這份文件裡最重要的一句話。** 記事本在中文 Windows 上另存時，
> 「編碼」預設可能是 **ANSI（cp950）**；包裡只要有一個中文字，存出來就變成
> Big5 位元組，而 Python 用 UTF-8 讀就死在
> `SyntaxError: Non-UTF-8 code starting with '\xe5' ...`。
> **2026-09-03 真的發生過**（那天早上把包改成不壓縮的純文字，31% 的位元組
> 是中文）。現在整份是 ASCII，所以記事本挑哪一種編碼都沒差 —— 而且有兩條
> 測試守著，其中一條真的用 cp950 存一次再解開。

`--list` 讓你在它寫任何檔案之前就看得到清單。想逐字讀過每個檔案的內容，
用下面那個 `--split` 的純文字分批版。
...
> ⚠ **2026-08-17 起「1 MB」不再是限制**：使用者搬運時直接複製 raw，
> 那條路跟檔案多大無關（見 [`../AGENTS.md`](../AGENTS.md) §2）。下面幾段
> 提到的分批與大小考量因此是**可讀性**的選項，不是必要的繞道。

為什麼純文字版要分批：**GitHub 不顯示超過 1 MB 的檔案**，而完全不壓縮的
純文字整包是 **7.6 MB** —— 在那台機器上根本點不開來複製，而且就算走 raw，
全選複製那麼大一份會卡到不能用（2026-09-03 實測）。每一批 < 420 KB。
...
`d4t_bundle.py`。出貨的那一份（`tools/release.py` 產的那個）是
**逐檔 lzma + base64**，2026-09-03 起。

那個格式要同時滿足三件事，而它們互相拉扯：

| | 為什麼 |
|---|---|
| **純 ASCII** | 記事本存 ANSI 也弄不壞（上面 §0a 那一段） |
| **夠小**（3.4 MB） | 瀏覽器全選複製不會卡 |
| **git delta 壓得動** | 逐檔壓，改一支模組只有那一段變 —— 每次 94 KB，不是 1.7 MB |

「逐檔」是關鍵：整包壓成一個流的話改一行就整份變樣。數字與那天踩的兩個坑見
[`../AGENTS.md`](../AGENTS.md) §2。

`--compress` 這個選項還在（＝更早的「整包壓成一個流」），解包程式**三種格式
都認得**、舊包照樣解得開，但**不要用它產出貨的那一份**。

```
python tools/make_text_bundle.py        # 產 d4t_bundle.py（逐檔壓，純 ASCII）
