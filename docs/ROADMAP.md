...
`docs/history/plans/F0-master-plan.md` 與 `SESSION_LOG.md` 四個地方，於是四份各自漂移。
要引用進度，連過來，不要再抄一份。
...
| **Region** | ✅ **收斂 2026-08-18，之後又收了兩次**（08-25 四張卡收成一張、09-02 兩張收成一張）。現在**只有一張卡**（`roi_reference`，畫面上叫 **ROI**），四種找法都是它的 method，輸出契約一致。七步的逐步紀錄見下面〈Region 段走過的七步〉 |
...
| **ADC** | ✅ **收斂 2026-08-28**。⚠ 這一格以前寫著「**一張卡都沒有，而且只分得出兩類**…整個 app 最大的功能缺口」—— 那**兩件事都不成立了**，而且答案跟當時猜的形狀不一樣。**多類別**由 `DecideSpec`（`let` 工作用數字 ＋ 判定樹 ＋ 任意 bin 編號）承接，F20–F27 做完；`__score__` 那個假節點沒有了。**卡片庫仍然零張卡，而那是對的** —— 判定不是「一張可以加好幾張的卡」，它是一份 recipe 的**一個**判定，所以 F50 把它做成畫布上一張**收合得起來的卡**（點開就是判定樹），而不是卡片庫裡的一個項目。**使用者不一定要用它**（沒有 `decide` 的 recipe 照跑）。剩下的 ML Classify 見下一列 |
| ML Classify | **Phase 2 後半**。吃已經匯得出來的 feature vector CSV；相依策略到時候再定 |

**Phase 2 的七段，六段已經收斂**（Input／Enhance／Region／Measure／ADC ✅；Output 的引擎與畫布兩半都做完了）。**Compare 是唯一還缺東西的**—— 缺的不是卡，是既有那幾張長 method（見上表那一列）。Measure 還欠的三件（離群旗標／Region Stats・FFT／`snr_map` 多來源）都卡在同一層：**`lot baseline`**（要看過整批才算得出來的那一層）。跨顆的**機制**已經有了（`Step.is_batch`），缺的是把它接進 Measure。


### Region 段走過的七步

**分三個階段，對應三種 mode（使用者定調 2026-08-17，照難度爬）**：

* **Region-1 Template ✅ 2026-08-18**（`roi_template` 改成「一張卡好幾個區域、一個區域好幾個矩形」，框在 Golden Cell 上**畫**出來 —— 含 multi add 一次長一整排等距的框；框映到 patch 時整片鋪過去，不再只取離缺陷最近的那一份。尾巴：模板過期健檢還沒做）
* **Region-2 Profile ✅ 2026-08-18**（`roi_cross`：三個下拉改圖示、點曲線就是挑材質，以及**單方向** —— 原本寫死「兩組正交條紋」是需求的形狀不是演算法的形狀，新的 `directions` 讓一張只有 line/space 的 patch 也定得出位置，做法是給沒在用的那一軸一根滿版的條紋，交會的幾何一行都不用改。**「三個框尺寸改成在影像上拖」使用者否決了** —— 那些框是逐顆的結果，拖它等於改輸出；可以用拖的只有所有 defect 共用的那一個物件）→ **兩張卡共用的兩件小事 ✅ 2026-08-18**（`drop_edge`／`edge_margin`：靠邊 n px 內的框自動拿掉 —— 壓在 patch 邊上的框量到的是半截的那一塊，而它照樣吐得出看起來正常的數字；**滿版的那一軸不算靠邊**、**缺陷那一塊永遠留著**。以及**疊框一個區域一個顏色**，跟模板編輯器同一組 —— 一張卡好幾個區域之後，全部畫成藍色就分不出哪塊是 ROI1）
* **Region-3 GDS**（**只做 RSEM**：單張 + KLARF，使用者定調 2026-08-18 —— GLAS 是瞄著 RSEM 大圖對的，patch 太小且 GLAS 那邊沒打通；那一刀讓「給 GLAS 的兩件必要需求」都不需要了。**匯出健檢 ✅**：`tools/check_glas_export.py` 印出可以貼出來的遮蔽報告，四條核心檢查是讀 GLAS 程式碼讀出來的。**第 1／2 步 ✅ 2026-08-18**：`tools/make_glas_export.py`（家用機的合成匯出）＋ `ingest/glas_export.py` 配對 ＋ `load_sidecar` 卡（附加檔住 `DefectItem.sidecars`，不進 `images` —— 混進去 `load_single` 會因為「一顆兩張」而全部載不進來）。順手修掉兩個安靜的錯：換匯出快取不失效、`load_raw` 會把三通道 label 合成灰階把 id 混掉。**第 3／4 步 ✅ 2026-08-18**：`roi_from_mask`（精確拆矩形 —— L 形的 bbox 會框到別的材質；`max_boxes` 預設 8192 是量出來的，真實一層是幾十到約五千個矩形，既有的 64 會安靜砍掉 95%）＋ Studio（`Open GDS export…`、`layers` 的表格、儀表）。新卡 `roi_from_mask` 吃 GLAS 的 label map；每個 layer 切成一堆小矩形 —— 站點的區域本來就都是矩形，所以那是等價不是近似）。出口契約已經留好（見 [`ARCHITECTURE.md`](ARCHITECTURE.md)），下游零改動）
* **Region-4 輸出統一 ✅ 2026-08-18**（三張找 ROI 的卡對**每一個區域**寫同一組五個數字 `present`／`boxes`／`area_px`／`clipped`／`edge_dropped`，使用者：「現在有點像大家資料結構不一樣」。前三個從 ctx 讀回來、不是卡片自己記一份；`clipped`／`edge_dropped` 整個家族共用；`present` 與 `boxes` 刻意會不一致 —— 退回整張圖那個保險是「有東西可以量，但它不是你要的那個」）
* **Region-5 單張大圖 ✅ 2026-08-18**（使用者要第四種「用重複結構鋪 ROI」的卡，**量過之後不必新卡**：Template 餵一張 1000×1000 就得到 625 個框、相位正確、61 ms，差別只在接哪一條流。修掉三個擋路的：`max_boxes` 64 → 8192（同 `roi_from_mask`）、`<name>_clipped` 在 Template 上原本恆為 0、對話框多一顆 `Use the image on screen`。`pattern_ref` 因此沒事做了，先收進 `HIDDEN_STEPS`、2026-08-20 刪掉）。**Region 段到此收斂 —— 四種找 ROI 的方法都在，輸出契約一致。**
* **Region-6 兩支收成一張卡 ✅ 2026-08-25（F29）**（使用者：「golden cell 跟 GDS 同樣重要而且他們要能在同張 card 裡（都是接區域 ROI 卡）」。`roi_from_mask` → **`roi_reference`**「Reference regions」，method = `repeating cells`（`algo/period` 量週期 → `algo/golden.tile_coords` 每一格一個框）／`layout layers`（原本的 GDS 那一支）。GDS 那一支**也吐 `_center` / `_others`** 了 —— 當初不吐的理由（大圖上缺陷不保證在正中央）沒有被推翻，當時的答案是 `pick="strongest"`（它去找）；F32 刪掉它之後，大圖上「找最異常」歸 GLV 的逐框比較，這張卡選 `pick="none"` 只放框。舊 recipe 走 `_migrate_roi_from_mask_into_roi_reference`。順手修掉一個安靜的：畫布的區域埠上限數的是**埠**不是區域，一層變三個名字之後 6 就等於「兩層」）。
* **Region-7 只剩一張卡 ✅ 2026-09-02**（使用者：「Mask from regions 這張 card 以及其相關功能請幫我拿掉（看不到此功能 card 用處），同時 ROI card 內剩下那一張就改名叫 ROI」。`roi_mask` 刪掉，連同它唯一的消費者 `normalize` 的 `use_within`（「Use only」）—— 沒有卡產得出 mask 流之後，那一格是 `image_key`（唯讀、只靠拉線填），留著就是一個**接不到東西的埠**。`roi_reference` 的 label 從「Reference regions」改成 **「ROI」**（`key` 與 feature 名一個都沒動）。代價量過而且很小：三份出貨 recipe、兩份 fixture、三份黃金值都沒用到，**黃金值三份逐項相同**。舊檔案：帶 `roi_mask` 的是一條 `unknown-step`（同 `pattern_ref` 先例），`use_within` 那一格由 `_migrate_drop_use_within` 拿掉 —— 不拿是「unknown parameters」，而那句話的意思是「這份檔案壞了」。⚠ `algo/histmatch.py` 的 `mask=` **不刪**：它是「量與套用分開」那個慣例的規範出處，`range_from` 走的是同一套（同 `algo/snr.py`／`algo/period.py` 那條規矩）。它擋的那個問題仍然存在（「MG 佔多少面積隨 crop 變」），而今天的答案是**量測段的區域** —— GLV 吃具名區域，量的本來就只有該看的那一塊）。
...
### Measure 段第三張：Focus index ✅ 多一個 IQI（F77，2026-09-02）

原本三個銳利度值（`focus_lapvar` / `focus_tenengrad` / `focus_fft`）不動，
多一個**站點指名的** OP-301 對焦分數 `focus_iqi`：切 64 塊 → 每塊 FFT 去背景 →
iFFT → 取分數最高的 30% 平均。演算法在 [`../d4t/core/algo/iqi.py`](../d4t/core/algo/iqi.py)，
四個步驟各一支函式（規格的驗收標準要）。

* **勾選不是四選一**（`metrics` 是一排膠囊）—— 這幾個可以同時要，而 IQI
  一顆 defect 要做 64 次 FFT，不該無條件付這筆錢。預設仍是原本三個，
  所以既有 recipe 與黃金值一個位元組都不動。
* **三份規格互相矛盾，而查下去發現它們不是在吵同一件事**：一份說壓低頻
  （去背景）、一份說 filter out high-frequency noise（去雜訊）—— 一個在
  頻譜最低端、一個在最高端，唯一同時成立的是**帶通**。而後者指的是一個
  真的失效模式：純高通下，一塊只有感測器顆粒的空背景拿 64 分（銳利 pattern
  是 7465），壓掉最高頻之後掉到 7 而 pattern 只掉 1.8 倍 —— 訊號背景比
  116× → 595×。**預設仍是純高通**（先能對上機台），帶通是一格參數。
* **梯度那一步在其中一份規格裡是空轉的**（算了，而選前 30% 是照 energy 選）。
  兩條路都留、預設不篩，而**無論走哪一條，梯度密度都變成一個看得見的數字**
  （`focus_iqi_pattern`）—— F19 那條規矩。
* **iFFT 由 Parseval 省得掉**，而輸入是實數影像所以「虛部」是浮點雜訊。
  仍然做（驗收要四個步驟），但有一條測試把兩條路釘在一起 ——
  哪天嫌慢，那條測試就是刪掉 Step 3 的許可證。

⚠ **待 owner 核對**：`Also wash out noise above` 預設 0（＝純高通）。
空背景區也會被評分的站點應該開到 30–40。
...
|---|---|---|
| 影像流 | 圓埠、實線 | 身分是 `(節點, 埠)` |
| 具名區域 | 菱形埠、虛線（**存在 `edges` 裡**，F42）| 名字在一條 route 上唯一（`duplicate-region` 是 error），所以線指的那張卡＝引擎給的那個框 |
| **特徵** | **沒有埠，但**有線**（淡的、推導出來的，F49/F50）| 見下 |
...
* **所以是「名字就是線」**：F17-② 之後每個數字的名字自己帶來源
  （`test_glv_mean`），「這個數字從哪來」看名字就知道 —— 不用埠。
...
### Output 段那一半 ✅ 2026-08-27（F49/F50）

> 「先做引擎，畫布那一半下一輪」那一輪到了，**而答案不是第三種埠**。

「不開新埠」的結論**沒有變**，變的是「所以畫布上就講不出來」那個推論 ——
它不成立。跨顆卡（`Write report` 的 `rank_by`、`plot_features`…）用**名字**吃
數字，而那些名字寫在它自己的參數格裡：所以滑鼠停在那張卡上，每一個名字亮一條
**淡線**回產出它的那張卡（`Step.feature_names_in` → `canvas.show_card_ghosts`）。

三件事讓它成立而不是變成第四套規矩：

* **跟判定樹菱形的幽靈線是同一支函式、同一個手勢**（`tree_scene.ghost_wires`）
  —— 使用者定調「不一致會有兩套準則，真正的使用者反而不知道怎麼用」，而
  「判定看得到來源、Output 看不到」正是兩套。
* **它不是一條邊**：不進 `recipe.edges`、不影響 `execution_order`、不進快取
  簽章、拉不動也剪不掉。唯一的真相是那張卡的參數，線是它的**投影**。
* **「誰產出這個名字」只有一份實作**（F51）：`verdict_features.bound_specs`，
  而它有一把尺 —— `tests/test_feature_names_match_the_engine.py` 拿引擎真的跑
  出來的名字去比對宣告層的答案。以前這件事有**三份**各自寫的實作。
...
## 畫布只有一套準則（F48–F57，2026-08-26 ～ 08-28）

這一段的起點是使用者的一句話：**「如果不一致，會有兩套準則，真正的使用者反而
不知道怎麼用。」** 配上他對整個 app 的心智模型：**「使用者是不會寫 code 的人，
但只要他看得懂每張 card 在幹嘛，那他就能用拉線去達到他想要的事。」**

所以這幾輪做的不是新功能，是**把畫布上每一種「例外」收掉**。

### 判定與 Output 真的變成 DAG 節點嗎 —— 量過之後：不用

使用者原本傾向把判定與 Output 變成真的節點（那樣就只有一套準則）。**評估之後
留下現有設計**，理由是量出來的代價太大而收益是可以用別的方式拿到的
（見 [`history/plans/F49-features-on-wires.md`](history/plans/F49-features-on-wires.md)）。

⚠ **而那一輪我有一個核心論點是錯的**，記在這裡因為它值得：我主張「拿掉一張
量測卡，判定會安靜地算錯」—— 實際刪一張卡去試，`unknown-feature` 警告**指名
道姓**地講了出來。使用者當時明講「不要因為我提出或我質疑就改變你的看法」，
而推翻它的不是他的質疑，是那次實測。

換來的兩件事（F48 建議 1＋2，使用者點頭）：淡的推導線（上一節）＋
判定卡上掛得住警告徽章。
...
| 做了什麼 | 為什麼 |
|---|---|
| **Output 的框拿掉**，「once per lot」變成卡片自己的一條腳帶 | 框的意思是「這幾個是一組」，真相卻是「跑的時間不一樣」—— 編碼錯了。而且那個框每一個拖曳 frame 重建一次，**留下殘影**（使用者回報）。`ui/output_band.py` 整支刪掉 |
| **判定收合成一張卡**（點開是判定樹）| 它是一份 recipe 的**一個**判定，不是可以加好幾張的卡 |
| **ADC 的 pre-filter 拿掉**，改成 Input 卡上可選的 **input code 篩選** | 「只跑 KLARF 某個 column code 的影像」問的是**資料**，那屬於 Input。`Step.item_filter` ＋ `batch.select_items`，而且**進快取簽章**（保守的方向：認錯了會回舊影像）|
| ROUTE BY 那塊**收起來不刪**（`scope.SHOW_ROUTE_BY`）| 不確定的時候先收起來，成本是零（CLAUDE.md §5 那張價目表）|
| Results 視窗**加一顆按鈕獨立叫得出來**（`Ctrl+Shift+R`）| 以前只有跑完才出現。⚠ 它一度掉進 Qt 工具列的 overflow —— 預設視窗寬度現在從 `toolbar.sizeHint()` 算 |
| 結果表**改得動 bin** | 使用者定調「可以」 |
| `GROUP_ALGO` 刪掉、ADC 那一段收掉 | 使用者定調 |

### 名字與數字只有一種寫法（F51 / F52 / F57）

* **「誰產出這個特徵」三份實作收成一份**（F51），並補上一把尺：拿引擎真的跑出來
  的名字去比對宣告層。
* **一個特徵值一種寫法**（F52）：以前同一個數字在畫面上有**六種**格式化函式，
  `99.995` 在結果表是 `100`、點進去變成 `99.995`。現在全 UI 只有
  `ui/numbers.py` 一支（5 位有效數字 —— 第一版提案的 `%.4g` **算過之後是錯的**，
  它就是把 `99.995` 印成 `100` 的那一個）。
* **撞名那句話看得見誰在讀**（F57）：判定正在讀 `glv_max` 跟沒有人讀它，
  以前逐字相同；被蓋掉的那張卡以前一個字都沒有。

### 畫布不能說謊，而「畫布怎麼想」要用畫布的定義去問（F55 / F56）

兩支都是使用者丟一份真實 recipe 進來才看得見的：

* **F55**：新的 `port-not-produced` lint。⚠ **它第一版對一條完全正確的線報錯** ——
  我拿 `Step.resolve_writes`（**引擎的**宣告）去回答「**畫布上**這張卡有哪些埠」，
  而畫布的答案是 `writes` ＋**原樣送出的** `reads`（F9-6 同進同出）。
  兩張表的差別**正好就是這條 lint 想守的東西**，所以量到的是差別本身。
* **F56**：自動排版把**有線的**卡排到它來源的左邊，線因此由右往左畫 ——
  在一張由左往右讀的畫布上那讀起來是「它先跑」。病根是隱含順序那一行沒有把
  自己寫的理由（「**沒有線的**卡片才照 route 排列」）當真。

兩件事一起換來一條通則，記在 `docs/PITFALLS.md`：**下游看起來沒事，不代表上游
沒事**（F56 直接呼叫 `layout_columns` 拿到的是**正確**答案，錯的是誰餵給它）。

### 工具：模板看得見了（F54）

`tools/show_template.py`（stdlib-only，公司機跑得動）把 recipe 裡的
`gc2:…` 模板存成 PNG —— 沒有 GUI 的機器上也檢查得了一份 recipe。
...
| Milestone | 狀態 | 內容 |
|---|---|---|
| F8 | ✅ | **純規則 ROI 定位 + mask 通道 + UI 第二波**（詳見 `SESSION_LOG.md` 逐輪紀錄與 `docs/history/plans/F8-rule-based-roi.md`）。已完成：`roi_cross`（條紋交會處放框、一鍵整批量 pitch）、~~`roi_mask` + Normalize `use_within`~~（**2026-09-02 兩個一起刪掉**，F74）、參數說明搬 tooltip、D 案版面（畫布佔中上、設定拿大頭、**畫布彈出視窗**兩窗互通）、右鍵平移、手動佈局保留（tidy 才重排）、route 虛線退役（排版仍吃隱含順序）、量測卡預覽疊區域框、`multi_choice` 參數型別（glv_stats 統計量用勾的）、subtract 預設 `b=ref`（patch 天生對齊；舊檔載入遷移補 `ref_aligned` —— **改預設值必附遷移**） |
