...
- **範例 recipe 全部拿掉**（`examples/` 已移除），Studio 上的「用範例資料試一次」
  ...
  （目前兩份：EBI↔API characterization、patch 的 dSNR 分布），走 `Open recipe…`
...
（見 [`plans/F49-features-on-wires.md`](plans/F49-features-on-wires.md)）。
