...
> **這一節講技術脈絡：拿了什麼、刻意沒拿什麼。授權那一面在
> [`LICENSING.md`](LICENSING.md)** —— 六個專案在 GitHub 上分別是什麼可見性、
> 有沒有 LICENSE 檔、d4t 裡哪十七支模組是 vendored 的。
> 一句話的版本：**六個都是同一個作者自己的**，所以 vendoring 沒有第三方
> 授權義務；真正有義務的是執行時的相依套件（PySide6 是 LGPL）。
