...
| **兩條線接到同一個入口，只有一條算數**（F9 Phase 4b 已擋） | 一條分支量出來的特徵**整批不見**，而且跑得完、`ok=True`、沒有任何錯誤訊息 —— 分數式子指到那個特徵才會爆 | 執行器對同一個入口是「取最後一條」（`graph.run_graph` 的 `feeding[-1]`），那不是畫面上看得出來的規則。兩層處理：lint `merge-into-one-port`（手寫的 recipe 也擋得到），以及**畫布上根本接不出來** ——拉第二條進同一個入口是「我改變主意了」，當場換掉舊的那條並講出換掉了誰。⚠ 換掉要等線**真的接起來之後**才做：會成環／接進 Input 卡的那條沒有落地，不該留下任何痕跡 |
| **Region 卡的輸出被當成影像流**（F9 Phase 4b 已修） | 從 `roi_cross` 的輸出埠拉線到量測卡，那張卡的 `source` 被設成 `"out"` —— 一條沒有人產出的流；狀態列還說「now works on out」，而 recipe 跑起來每一顆都失敗 | Region 卡不寫影像流，它的輸出埠是佔位的 `out`。線本身有意義（先定位、再量），錯的只有「順手把那張卡改成做 out」—— `studio._real_stream` 只擋改參數那一半，並講清楚「它不產生影像流，所以那張卡看的還是原來那條」|
| **在 QGraphicsItem 的事件處理函式裡同步發訊號**（2026-08-16 已修） | 畫布上按那顆「×」**整個行程閃退**（exit 139，沒有任何錯誤訊息）| 接收端改 model → listener 呼叫 `set_nodes()` → 它第一件事是 `scene.clear()`，把**正在處理這個滑鼠事件的那條線自己**刪掉；事件處理函式回去時 Qt 去碰一個已經釋放的 C++ 物件。用 `QTimer.singleShot(0, …)` 排到下一輪事件迴圈再發。⚠ 這不只是剪線 —— **任何會導致重建場景的訊號都不准在 item 的事件處理函式裡同步發**。Phase 4a 之前剪不掉所以不重建，才一直沒炸出來 |
| **推導出來的線剪不掉**（F9 Phase 4a 已修） | 畫布上那顆「×」按下去，狀態列說「Disconnected」，而圖一點都沒變、線還畫在原地 —— 說做了卻沒做 | 主幹以前是從 `node_order` 的相鄰對**推導**的，而 × 只從 `model.edges` 刪東西，推導出來的線根本不在那份清單裡。規矩改成**一份 recipe 只要有一條顯式的線，引擎就不再推導主幹**（`graph._compile_recipe` 的 `backbone_is_explicit`），而 `add_edge` /`remove_edge` 動線之前先 `materialise_backbone()` —— 不然拉第一條線的瞬間其餘的卡會全部變成孤兒。新 lint `no-upstream` 在跑之前就講出哪張卡沒人接 |
