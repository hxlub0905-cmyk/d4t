...
| **怎麼用模擬資料產生器**（給使用者的操作手冊：貼一張 GC → 一整批 RSEM／patch，含配對的乾淨版與遮罩）| [`docs/USING-SIMGEN.md`](docs/USING-SIMGEN.md) | 要動 `python -m d4t simgen` 的介面、`tools/make_lot_from_gc.py` 的參數或擬真預設之前 |
