#!/usr/bin/env python3
# ADEPT 測試跑法 — authored 2026-08-15.
...
為什麼需要這支（實測數字，2026-08-15）
--------------------------------------
同一套測試、同一台機器：

===================================  ========
``pytest -q``（一個行程跑完全部）      **> 11 分鐘**
每個檔案各自一個行程（這支）           **約 2.5 分鐘**
===================================  ========

差距在 Qt。全套裡有 25 支 UI 測試檔，一個行程跑的話，前面每一支建立的
...
⚠ **這支不取代 CI。** CI 跑的是不加參數的 ``pytest -q``（三個 Python 版本），
因為「在一個行程裡跑也要能過」本身是一條性質：測試之間不該互相汙染。
這支是給**開發迴圈**用的，讓「跑一下確認沒弄壞」從十幾分鐘變成兩分半。
...
#: 開 Qt 視窗的測試檔前綴（跟 ``tests/conftest.py`` 的 slow marker 同一條規則）。
UI_PREFIX = "test_ui_"
